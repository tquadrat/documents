...
%To generate the index: "makeindex -s JavaCodingConventions.ist JavaCodingConventions"

\begin{document}
\isodate
\input{settings/title}
\input{settings/dedication}

\input{settings/toc}

\input{introduction/Chapter}
\input{formattingCode/Part}
\input{naming/Part}
\input{comments/Part}
...
%------------------------------------------------------------------------------
\chapter{Inline vs. Methods}\label{sec:InlineVsMethods}
Discuss whether it is better to move code to a (private) instead of keeping it inline.
...
A proper error handling is crucial for any code, no matter in which type of program it lives – but the definition of \textit{proper} depends significantly on the type of the final product, as it was defined \hyperref[sec:TypesOfProducts]{above}.
...
\subsection{Domain Specific Exceptions}
Each domain (component, module, area of operation, name it …) should have its \bdq{Domain specific Exception} – or, more precisely, its own, specific hierarchy of unchecked or checked Exceptions. For internal purposes, it could have also another hierarchy of checked Exceptions, but these should not get much visible outside that domain.
 
Except InvalidArgumentException and exceptions derived from that (\bdq{validation exceptions}), a domain or component may never emit an exception that is not defined by itself (except NPE, but this may not occur anyway). All other exceptions (exceptions, not errors!) must be caught and handled or wrapped. InvalidArgumentExceptions are thrown only by the API methods of the component as response of an invalid argument to that method. IAE thrown by other component used by this component must be wrapped and delegated or handled internally.
 
To illustrate that, we assume that we defined the component \bdq{Printing} that is responsible for the generation of documents for paper-like output and their distribution. It defines the exception class \bdq{PrintingException} (or \bdq{PrintingRuntimeException}, if you like that better), derived from java.lang.RuntimeException. If will directly extend RuntimeException, or  if Printing has a parent component, it will extend that component’s domain specific exception. Perhaps you have defined an application exception (like our PRRuntimeException), so the domain specific exceptions will extend that.
 
This \bdq{domain root exception} allows all consumers of the respective component to catch all specific exceptions from that component with a single catch:
...
{
    final var message = """
       Printing Operation failed with an Exception
       Recipient Id: %s
       Origin: %s
...
In this sample, \bdq{recipientID} and \bdq{dataSource} will enhance the information that is already provided with the PrintingException object; probably the Printing component was called with the data itself (not with the dataSource), and the recipient is not part of the generated document.
 
Printing now defines problem specific exceptions derived from PrintingException; this allows the consumers to respond to specific problems in a specific manner: if the Printing component throws a PrintingDeviceException, the consumer knows that there is a problem with the output device and can send an email to the support staff to inform them that the printer needs maintenance. A PrintingBadDataException tells the consumer that the data it provided was faulty, and a PrintingTooMuchDataException (probably derived from PrintingBadDataException) signals that too much data should be printed. An InvalidPrintingFormatException would indicate that the output description is not valid, and a NoOutputDeviceException tells that there is no printer with the given name.
 
When the consumer wants to respond to all these exceptions differently (and it should do so! At least it should provide different data to the logging), the code snippet above would look like that below. Of course, we could always provide all information and leave it to the support engineer to identify what is relevant. But that would also mean that any kind of self-healing would get unnecessary complex.
...
catch( final NoOutputDeviceException ) // Self-Healing
{
    final var message = """
       Printing Operation failed because provided device does not exist
...
catch( final InvalidPrintingFormatException )
{
    final var message = """
...
catch( final PrintingTooMuchDataException )
{
    final var message = """
...
catch( final PrintingBadDataException e )
{
    final var message = """
       Printing Operation failed with an Exception
       Recipient Id: %s
...
This decision (\bdq{either wrap and throw or log and terminate}) has to be taken for each and every call.
 
Using an unchecked hierarchy means that the consumer can completely ignore the exception handling for the PrintingException (and leave it to its own consumers). It also means that the API methods for the PrintingComponent do not need to have a throws clause, to declare the thrown exceptions. A sample for this approach can be found with the java.util.regex package (the regular expression handling in Java). The other approach, to use checked exceptions, can be found in java.io that defines IOException as the \bdq{domain specific exception} and derives exceptions like FileNotFoundException from that. Checked exceptions enforce that each call is somehow wrapped in a try-catch block, otherwise the code does not compile. This forces the attention for the error handling on each caller but can bloat the code. One can say bloated code is much better that code without proper error handling, but when it is too bloated, the code is difficult to read and prone to get broken when amended.
 
Some words regarding the calls to terminate() vs. abort(): assume we use the code snippet in a loop, where each cycle uses different data to be printed. A PrintingBadDataException means that the current data is somehow corrupted, but we can proceed with the loop (terminate the current process and load the next dataset), but an InvalidPrintingFormatException or a PrintingDeviceException means that no printing is possible at all and we abort the loop.
 
There are libraries that use \bdq{Error} (domain specific implementations of java.lang.Error) to signal the abort condition, while \bdq{Exception} means: \bdq{you can try again with different (valid) input data}. I don’t like this approach as it is a widely accepted best practice not to catch Error.
...
Your code will emit an exception\index{exception} when it encounters an error condition of some kind. Exceptions are discussed in detail in \jls\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:Exceptions}; I just repeat the basics here.
...
\begin{itemize}[nosep]
\item{\lstinline|java.lang.Exception|\autocite{ORACLE_DOC_EXCEPTION_CLASS}\index{java.lang!Exception}\index{exception!checked exception}}
\item{\lstinline|java.lang.RuntimeException|\autocite{ORACLE_DOC_RUNTIMEEXCEPTION_CLASS}\index{java.lang!RuntimeException}\index{exception!unchecked exception}}
\item{\lstinline|java.lang.Error|\autocite{ORACLE_DOC_ERROR_CLASS}\index{java.lang!Error}\index{exception!error}}
\end{itemize}

\begin{center}
% Generated by "dot -Tjpg -y exception.gv > exception.jpg"
	\includegraphics[scale=1.0]{exception}
\end{center}

The different kinds of exceptions are discussed in \autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:KindsOfExceptions}.

Exception classes that extend \lstinline|java.lang.Exception|\index{java.lang!Exception} are so-called \textit{checked} exceptions\index{exception!checked exception}\index{checked exception|see{exception}}; if a method wants to throw one of these, it has to declare that with the \lstinline|throws| clause. The class \lstinline|java.lang.RuntimeException|\index{java.lang!RuntimeException} is a direct subclass of \lstinline|java.lang.Exception|\index{java.lang!Exception} and the superclass of all \textit{runtime}\index{runtime exception|see{exception}} or \textit{unchecked}\index{exception!unchecked exception}\index{unchecked exception|see{exception}} exceptions – declaring these is not mandatory.

While an ordinary program can \textit{potentially} recover from an error condition signalled by an \lstinline|Exception|\index{java.lang!Exception}\index{exception} instance, it is assumed that instances of the class \lstinline|java.lang.Error|\index{java.lang!Error}\index{exception!error} and its subclasses will indicate unrecoverable error conditions, causing the program to abort.
...
\begin{center}
\includegraphics[scale=0.6]{TestAbortMain}
\end{center}
...
\begin{center}
\includegraphics[scale=0.5]{TestAbortRun}
\end{center}
...
To avoid that you repeat yourself on validating the arguments (refer to the \ngref~ bullet point \ref{lst:ZoP:DRY}) you should consider to provide a utility class\index{class!utility class} as central location for validation methods; using (domain) value types would be an alternative (refer to chapter \tqfullvref{sec:ValueTypes}).}
...
As said in \jls\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:Assert}:
\begin{quote}
...
\begin{center}
% Generated by "dot -Tjpg -y collection.gv > collection.jpg"
\includegraphics[scale=1.0]{collection}
\end{center}
...
\begin{center}
% Generated by "dot -Tjpg -y map.gv > map.jpg"
\includegraphics[scale=1.0]{map}
\end{center}
...
Another issue is that you have to validate the provided person ids before storing them. Most probably this happened already elsewhere, perhaps already several times, but the constructor is public and you have to do it here again. Of course you can provide (and you should do!) the validation method through a utility class\index{class!utility class}, but it still costs time and CPU cycles.
...
\begin{center}
% Generated by "dot -Tjpg -y reflection.gv > reflection.jpg"
\includegraphics[scale=1.0]{reflection}
\end{center}
...
\item{When using modules\autocite{OPENJDK:ProjectJigsaw}\index{Java Platform Module System}, the results of calls to the Reflection\index{reflection} APIs get unpredictable.
...
\begin{itemize}[nosep]
\item{For XML\index{XML} parsing: JAXB and its predecessors\index{JAXB} \autocite{ORACLE_DOC_JAXB, ECLIPSE_XML_BINDING}}
\item{For JSON\index{JSON} parsing: GSON\index{GSON} \autocite{GOOGLE_GSON} and Jackson\index{Jackson} \autocite{FasterXML:Jackson}}
\item{For WebServices: JAX\_WS\index{JAX\_WS} \autocite{IBM_JAX_WS, ECLIPSE_XML_SERVICES}}
\item{For ORM\index{ORM}: JPA\index{JPA} \autocite{ECLIPSE_JPA} and Hibernate\index{Hibernate} \autocite{HIBERNATE_ORM}}
\item{For command line handling: ARGS4J\index{ARGS4J} \autocite{Kawaguchi:args4j_github}}
\end{itemize}

In fact, some of these frameworks/libraries are only possible because annotations had been introduced with Java~5: the annotations allow to apply meta information to classes above the information provided by the class definition itself.
...
And if you have to provide a mapper from an internal data exchange format (that is not XML\index{XML} or JSON\index{JSON}~…), you should consider to write an annotation processor\index{annotation processor} (AP) that generates the respective code – in particular if you are in control of the full build process for the project that is intended to use that mapper.
...
\subsection{The Method \bsq{clone()}}\label{sec:Clone}
The Javadoc for \lstinline|java.lang.Object::clone|\index{java.lang!Object!clone()} \autocite{ORACLE_DOC_OBJECT:clone} says:

\begin{quote}
``Creates and returns a copy of this object. The precise meaning of \bsq{copy} may depend on the class of the object. The general intent is that, for any object \lstinline|x|, the expression:
\begin{lstlisting}
x.clone() != x
\end{lstlisting}
...
will be also true, but these are not absolute requirements. While it is typically the case that:
\begin{lstlisting}
x.clone().equals(x)
\end{lstlisting}
...
By convention, the object returned by this method should be independent of this object (which is being cloned). To achieve this independence, it may be necessary to modify one or more fields of the object returned by \lstinline|super::clone| before returning it. Typically, this means copying any mutable objects that comprise the internal \bsq{deep structure} of the object being cloned and replacing the references to these objects with references to the copies. If a class contains only primitive fields or references to immutable objects\index{immutable type}, then it is usually the case that no fields in the object returned by \lstinline|super::clone| need to be modified.''
\end{quote}

When instances of your class should be cloneable, it first has to implement the interface \lstinline|java.lang.Cloneable|\index{java.lang!Cloneable} \autocite{ORACLE_DOC_CLONEABLE_INTERFACE}, and then you need to override the method \lstinline|clone()|\index{java.lang!Object!clone()}; usually, this method is protected and it declares to throw a \lstinline|java.lang.CloneNotSupportedException|\index{java.lang!CloneNotSupportedException} \autocite{ORACLE_DOC_CLONENOTSUPPORTEDEXCEPTION_CLASS}. The default implementation does nothing else that throwing that exception when called.
...
%------------------------------------------------------------------------------
\chapter{String Handling}\label{sec:StringHandling}

\section{Text Blocks}\label{sec:TextBlocks}
Text block\index{text block}

\lstinline|String::indent|\index{java.lang!String!indent()} \autocite{ORACLE_DOC_STRING:indent}
...
For the \bsq{traditional} pattern (using \verb#try-catch-finally#\index{try-catch-finally} as shown in the first example of the previous chapter) this could mean that the first exception would be ‘supplanted’ by the exception issued by \lstinline|close()|, called in the \lstinline|finally| block. For sure, in a \lstinline|catch| block the original cause could be logged before the code in the \lstinline|finally| block will be executed, but usually only checked exceptions (and \bdq{expected} ones) are covered this way.

Together with \verb#try-with-resources#\index{try-with-resources}, a new feature was introduced to the language: the \textit{suppressed} exception\index{exception!suppressed exception}. This deals with the problem described above.

So if the \lstinline|try| block throws an exception (for our example, it would be most probably an \lstinline|IOException|\index{java.io!IOException}) and the \lstinline|AutoCloseable.close()|\index{java.lang!AutoCloseable!close()} will fail with an exception, too, the latter one will be added to the first one as a \bdq{suppressed exception}\index{exception!suppressed exception} by the JVM.
...
\begin{center}
% Generated by "dot -Tjpg -y trycatchfinally.gv > trycatchfinally.jpg"
\includegraphics[scale=1.0]{trycatchfinally}
\end{center}

For \verb#try-with-resources#\index{try-with-resources} the execution sequence looks like 

\begin{center}
% Generated by "dot -Tjpg -y trywithresources.gv > trywithresources.jpg"
\includegraphics[scale=1.0]{trywithresources}
\end{center}
...
The two following chapters (“\nameref{sec:Lifecycle}” and “\nameref{sec:PostProcessing}”) show some additional ways to make use of \verb#try-with-resources#\index{try-with-resources}.
...
A workaround is to use a \verb#try-finally#\index{try!finally} block with the cleanup (usually a call to a method \lstinline|close()|) in the \lstinline|finally|\index{try!finally} block. But to often we have seen that in the run of modifications and/or corrections (refactorings) suddenly the \lstinline|finally|\index{try!finally} block and/or its contents had been removed (\bdq{optimised away}). Another problem were exceptions that are triggered from within a finally block.

Now, with the \verb#try-with-resources#\index{try-with-resources} feature, we can have \bdq{Lifecycle} classes; they are still not that easy to use than a C++\index{C++} class with real destructors, but we can come close.
...
\subsubsection{Post-Processing}\label{sec:PostProcessing}
...
\subsubsection{Principles}
\begin{enumerate}[label=P\arabic*.]
\item{A method has just one single \textit{regular} return point\footnote{Sometimes also referred to as “exit point”}\index{return point}\index{exit point|see{return point}}.

This is either \textit{after} the last statement for a \lstinline|void| method (in PASCAL\index{PASCAL}, such a method was called a ‘procedure’\index{procedure}), or the very last statement of the method is the \lstinline|return| statement with the return value (a ‘function’\index{function} in PASCAL\index{PASCAL}).}

\item{Additional \textit{irregular} exit points\index{return point} have to be thrown exceptions.

That's the reason why exceptions have been introduced.}

\item{The name of the variable that holds the return value is always \lstinline|retValue|.
...
\begin{center}
% Generated by "dot -Tjpg -y FlowChart.gv > FlowChart.jpg"
\includegraphics[scale=1.0]{FlowChart}
\end{center}

This is a relatively simple function (method) that has only one regular exit point\index{return point}, but it is nevertheless quite confusing when presented as a standard flow-chart. In particular it is not easy to follow the program flow, and you cannot see immediately that there is really only one return point\index{return point} in it.

So we soon started to use ‘structograms’\index{structogram} or Nassi-Shneiderman diagrams\autocite{WIKIPEDIA:NSD}\index{Nassi-Shneiderman diagram|see{structogram}}. As such a structogram, the same code looks like this:

\begin{center}
% Generated by "java -jar nsdg.jar ./NassiShneiderman.xml"
\includegraphics[scale=0.38]{NassiShneiderman}
\end{center}

The biggest advantage of such a ‘structogram’\index{structogram} is that you can take the code directly out of the diagram, because you have to read it strictly from top to bottom, and hence they have just one exit point\index{return point}\footnote{Exceptions did not yet exist when these diagrams had been invented, but as shown, they can be easily implemented.} at the bottom. The resulting Java code looks like this:
\index{org.tquadrat.foundation.util!StringUtils!breakText}\index{java.util.stream!Stream}\index{java.lang!String}\index{java.lang!CharSequence}\index{org.tquadrat.foundation.exception!ValidationException}\index{java.util.stream!Stream.Builder}\index{java.util.stream!Stream!builder()}\index{org.tquadrat.foundation.util!StringUtils!splitString()}\index{org.tquadrat.foundation.lang!Objects!requireNonNullArgument()}\index{org.tquadrat.foundation.util!StringUtils!isEmptyOrBlank()}\index{java.util.stream!Stream.Builder!add()}\index{java.lang!StringBuilder}\index{java.lang!String!split()}\index{java.lang!String!isEmpty()}\index{org.tquadrat.foundation.util!StringUtils!isNotEmpty()}\index{java.lang!StringBuilder!add()}\index{java.lang!StringBuilder!setLength()}\index{java.util.stream!Stream.Builder!build()}
\begin{lstlisting}[numbers=left,caption={StringUtils::breakText}]
public static final Stream<String> breakText( final CharSequence text, final int lineLength )
{
    if( lineLength < 1 ) 
        throw new ValidationException( format( "Line length size must not be zero or a negative number: %d", lineLength ) );
...
It is easy to recognise where the elements of structogram\index{structogram} ends up in the code. Therefore it is also quite easy to see in the structogram which impact a modification of the code would have, while the usual flow-chart would not help here.

So thinking how your code would look like as a Nassi-Shneiderman diagram\index{structogram} helps to protect it from bugs that are introduced by code changes. One requirement for code that can be represented as a structogram\index{structogram} is, that there is just one single regular exit point\index{return point}.\footnote{IntelliJ IDEA can be configured to emit an error for methods with more than a configured number (the default is 1) of return points.}

A bad sample are the \lstinline|equals()|\autocite{ORACLE_DOC_OBJECT:equals}\index{java.lang!Object!equals()} methods that are generated by the IDEs:
\index{java.lang!Object!getClass()}\index{org.tquadrat.foundation.lang!Objects!equals()}
\begin{lstlisting}[numbers=left]
\\ DISCOURAGED!!
@Override
public boolean equals( final Object o )
{
    ...
    final var other = (MyClass) o;
    return m_Boolean == other.m_Boolean && Objects.equals( m_String, other.m_String );
}
\end{lstlisting}

Obviously you cannot create a structogram\index{structogram} for the code above.

The chapter \tqvref{sec:EqualsAndHashCode} describes how to implement \lstinline|Object::equals|\index{java.lang!Object!equals()}; according to that description, the code above would look like this:
\index{java.lang!Object!getClass()}\index{org.tquadrat.foundation.lang!Objects!equals()}
\begin{lstlisting}[numbers=left]
@Override
public final boolean equals( final Object o )
{
    ...
    if( !retValue && o instanceof MyClass other )
    {
        retValue = getClass() == other.getClass() )
            && m_Boolean == other.m_Boolean 
            && Objects.equals( m_String, other.m_String );
    }
...
\begin{center}
% Generated by "java -jar nsdg.jar ./equals.xml"
\includegraphics[scale=0.5]{equals}
\end{center}

As already said in chapter \tqfullvref{sec:NamesForLocalVariables}, the name of the local variable that holds the return value is always \lstinline|retValue|; only when the method returns just an attribute, the field name could be used instead. And it is also not necessary to assign \lstinline|this| to \lstinline|retValue| for method chaining. Hence these are all valid samples:
\index{java.lang!String!formatted()}\index{java.io!PrintStream}\index{java.io!PrintStream!print()}
\begin{lstlisting}[numbers=left]
// A Getter method
public final String getAttribute() { return m_Attribute; }

// An accessor method
public final String attribute() { return m_Attribute; }

// Returning 'this' for method chaining
public final MyClass attribute( final String value )
{
    m_Attribute = value;
...
// Any other methods
public final String compose( final String pattern )
{
    final var retValue = pattern.formatted( m_Attribute );
    
    //---* Done *----------------------------------------------------
    return retValue;
}   //  compose()

public final void print( final PrintStream destination )
{
    destination.print( m_Attribute );
}   //  print()
\end{lstlisting}
...
Most lambdas\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:LambdaExpressions} are just one liners, and if they return something, it will be the return value of that line:
\begin{lstlisting}
...
And you should write those complex lambdas also with just a single return point\index{return point}!

%------------------------------------------------------------------------------

\subsection{‘case’ Results}\label{sec:CaseResults}
With Java~12, \lstinline|switch| expressions\autocite{ORACLE_DOC_SWITCHEXPRESSIONS} have been introduced into the language. Additional details can be found in chapter \ref{sec:TheSwitchStatement}.

Basically, a \lstinline|switch| expression looks like this\footnote{Also refer to chapter \tqvref{sec:NewSwitch}.}:
\begin{lstlisting}[numbers=left]
<result> = switch( <selector> )
{
    case null -> <nullExpression>;
    
    case <switchlabel1> -> <expression1>;

    case <switchlabel2>, <switchlabel3> -> <expression2>;
...
\begin{lstlisting}[numbers=left]
public final <T> boolean isEmpty( final T arg )
{
    final var retValue = switch( arg )
    {
        case null -> true;
        case CharSequence charSequence -> charSequence.isEmpty();
        case Collection<?> collection -> collection.isEmpty();
        case Map<?,?> map -> map.isEmpty();
        case Enumeration<?> enumeration -> !enumeration.hasMoreElements();
        case Optional<?> optional -> optional.isEmpty();
        case Iterator<?> iterator -> !iterator.hasNext();
        case Throwable throwable -> isNull( throwable.getCause() );
        ...
        default ->
        {
            if( !arg.getClass().isArray() ) throw new IllegalArgumentException();
            
            final var result = Array.getLength( arg ) == 0;
...
Same as for a lambda, you use \lstinline|result| as the name of the local variable that holds the return value. This means that a collision could occur when using a complex lambda within a \lstinline|case|\index{switch!case} block~… – if you really encounter this problem, you need to think about the structure of your code anyways – refer to \tqfullvref{sec:TheSwitchStatement} for more details.

And of course, there is only one \lstinline|yield|\index{switch!yield} statement per \lstinline|case|\index{switch!case} block, and it has to be the last statement in that block.
...
\ref{sec:CaseResults}
\autocite{ORACLE_DOC_PATTERNMATCHING}

\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpression, ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpressionBlock, ORACLE_DOC_SWITCHEXPRESSIONS}
...
\section{The \bsq{for} Statement}\label{sec:TheForStatement}

\ref{sec:ForStatements}
\ref{sec:CaseResults}
\autocite{ORACLE_DOC_PATTERNMATCHING}

\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:For, ORACLE_DOC_LANGUAGE_SPECIFICATION:ForBasic, ORACLE_DOC_LANGUAGE_SPECIFICATION:ForEnhanced}
...
\section{The \bsq{while} Statement}\label{sec:TheWhileStatement}

\ref{sec:WhileStatements}
\ref{sec:CaseResults}
\autocite{ORACLE_DOC_PATTERNMATCHING}

\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:While}

%------------------------------------------------------------------------------

\subsection{The Problem with an Empty Loop Body}\label{sec:EmptyWhile}
For an empty \lstinline|for|\index{for!empty loop}
\begin{lstlisting}
for( <initialization>; <condition>; <update> );
\end{lstlisting}
all the work is done in the initialisation and/or the update clauses\footnote{It is even possible to get something done in the condition clause, as a side effect}, and usually, this is honoured by the compiler.

An empty \lstinline|while| loop\index{while!empty loop}
\begin{lstlisting}
while( <condition> );
\end{lstlisting}
could do work only as a side effect of the condition clause, and this is often not visible to the compiler. With the result that the empty loop is eleminated during the optimisation process.

So the code snippet below, where the current thread should wait until another thread changes a status, will not work as expected:
\begin{lstlisting}[numbers=left]
// Will not work as expected!
final void run()
{
    while( getOtherThreadStatus() == BUSY );
    
    // Do something …
}   //  run()
\end{lstlisting}
The compiler will just ignore line~4, so that \lstinline|run()| will not wait for that other thread.

A nasty fact is that the empty loop could be eleminated also by the JIT\index{JIT} at runtime.

Consequently you should not use \lstinline|while|\index{while!empty loop} loops with an empty body.
...
MENTION PATTERN MATCHING HERE!!!

In this document, I mentioned the \lstinline|switch| statement\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:Switch} a first time in chapter \tqfullvref{sec:SwitchStatements}, where the formatting of the source code is discussed, and a second time in chapter \tqvref{sec:CaseResults} that deals with return values.

Java inherited the problematic syntax of the \lstinline|switch| statement from C\index{C}; whether, when and how it should or could be used was always discussed controversally. There are projects that even banned the use of \lstinline|switch| completely in their coding guidelines, because it is error prone.

Finally, a series of JEPs were launched to address the issues with the \lstinline|switch| statement (JEP~325\autocite{JEP325}, JEP~354\autocite{JEP354} and JEP~361\autocite{JEP361}). Java~12 introduced the ‘new’ \lstinline|switch| as a preview \autocite{JEP325:Specification}, since Java~14 it is an official part of the language.

Some sources (e.g. \autocite{ORACLE_DOC_SWITCHEXPRESSIONS}) refer to the different syntax variants by the form of the \lstinline|switch| labels as ``\verb#case L:#'' (‘old’) and ``\verb#case L ->#'' (‘new’). 

\input{Import_SwitchTypesTable}

As we have two different forms of the \lstinline|switch| statement now, it is worth to define some coding guidelines for it.

%------------------------------------------------------------------------------

\subsubsection{Principles}
\begin{enumerate}[label=P\arabic*.]
\item{Prefer the ‘new’ \lstinline|switch| over the ‘old’ one.

One motiviation for the JEPs mentioned above had been the error prone syntax of the original “\verb#case L:#” \lstinline|switch| statement; this is fixed by the new “\verb#case L ->#”form – but only when it is used.}

\item{Using \lstinline|switch| \textit{expressions} can make your code much more concise.}

\item{\lstinline|case| blocks should be short.

No matter if you are using the old or the new form, keep the code in the \lstinline|case| blocks short and simple. Move the code to a \lstinline|private| method, if it gets too long or too complex or both.}

\item{Avoid ‘fall-through’!

The new form does not allow ‘fall-through’, hence this is only relevant for the “\verb#case L:#” form; ‘fall-through’ is the main reason why the old \lstinline|switch| is considered to be error prone.}

\item{Although it looks like one, a \verb#switch rule# is no lambda.

That means that local variables that are referenced in a \verb#switch rule# expression do not have to be effectively \lstinline|final|.}

\item{Never use the “\verb#case L:#” label for a \lstinline|switch| expression!

Although it works, the old label form for a \lstinline|switch| expression looks just confusing – and is in a similar way error prone as for a regular \lstinline|switch| statement.}
\end{enumerate}

%------------------------------------------------------------------------------

\subsection{Advantages of the new Syntax}
The biggest advantage of the new syntax with the “\verb#case L ->#” label is that it allows to avoid the problems of the original syntax with the “\verb#case L:#” labels; previously, you wrote
\begin{lstlisting}[numbers=left]
switch( selector )
{
    case 1: 
...
\end{lstlisting}
Paranoid people (like me) would have added a \lstinline|break| after \lstinline|executeDefault()|, but the syntax rules does not require that.

If – for whatever reason – the \lstinline|break| in line~5 will get lost, it means that for \lstinline|selector == 1|, both \lstinline|executeOne()| and \lstinline|executeTwo()| are invoked. Same if you add
...
\end{lstlisting}
for all values that are not \verb#1#, \verb#2#, or \verb#3# (without being paranoid, of course): after \lstinline|executeDefault()|, \lstinline|executeThree()| is called.

With the new syntax, you do not need the \lstinline|break| statement any longer:
\begin{lstlisting}[numbers=left]
switch( selector )
{
    case 1 -> executeOne();
...
The \lstinline|switch| expressions\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpression, ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpressionBlock, ORACLE_DOC_SWITCHEXPRESSIONS} came with the new syntax, too, but it is not dependend on it; instead of writing
...
replacing the \lstinline|break| that usually terminates a branch by \lstinline|yield| with the result value.

For \lstinline|switch| expressions, my recommendation is to use always the “\verb#case L ->#” syntax.

%------------------------------------------------------------------------------

\subsection{‘switch’ Expressions}
As already mentioned before, you can now have a \lstinline|switch| expressions\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpression, ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpressionBlock, ORACLE_DOC_SWITCHEXPRESSIONS}; that means that each \lstinline|case| block returns a result value. Based on the recommendation from the previous chapter, that looks like this:
\begin{lstlisting}
final var result = switch( selector )
{
    case 1 -> retrieveOne();
...
Obviously the result type of each branch has to be compatible to those of the other branches (if it is not even always the same). If that is not really visible from your code, you should consider to use a concrete type for the receiving variable instead of \lstinline|var|. 
...
\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpression}
\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpressionBlock}
...
\end{lstlisting}
Or you put the common code into a \lstinline|private| method and call that from both branches. 
...
The \lstinline|break| in the \lstinline|default| branch is redundant, but it prevents a fall-through error if later another \lstinline|case| branch is added – although it should be assured that the \lstinline|default| branch is always the last branch in the \lstinline|switch| statement. Of course no \lstinline|break| is required when the branch is left by throwing an exception.\footnote{A \lstinline|break| would be also obsolete when the branch is left through a \lstinline|return| statement, but in that case, the \lstinline|return| would not be the last statement in the method, and this is another requirement!}
...
\section{Modularisation}\label{sec:Modularisation}
\section{-------------------------------------------- Proceed from here!}
\lipsum[5]
\tqfullvref{sec:ModuleDefinition} and modules in general are discussed in \tqfullvref{sec:Modularisation}
name for a module: \tqfullvref{sec:Modules}
...
**Java Lambdas — A Concise Overview**

**What are they?**
A lambda expression is a compact way to represent an instance of a functional interface (an interface with exactly one abstract method) as an anonymous, inline block of code. 
Syntax: \lstinline|(parameters) -> expression_or_block|. They were introduced in Java 8 (2014).

**Why were they added?**
- To enable functional-style programming on the JVM, complementing Java's object-oriented model.
- To reduce the verbosity of anonymous inner classes, especially for simple callbacks (`Runnable`, `Comparator`, event listeners, etc.).
- To power the new Streams API — lambdas are the natural way to pass behavior (filter/map/reduce predicates) into stream operations.
- To make APIs more expressive without breaking backward compatibility (achieved via default methods on interfaces).

**Advantages**
- **Conciseness** — replaces boilerplate anonymous classes with a few characters.
- **Readability** — intent (what to do) is clearer than mechanism (how an object is constructed).
- **Enables Streams/functional APIs** — `map`, `filter`, `reduce`, `forEach`, etc. become practical.
- **Deferred execution** — behavior can be passed around and invoked later (useful for lazy evaluation, callbacks, event handling).
- **Effectively-final capture** — safe, predictable closure semantics over local variables.

**Disadvantages**
- **Debugging is harder** — stack traces for lambdas are less readable (synthetic method names, no meaningful frame).
- **No explicit `this`** — lambdas share enclosing scope's `this`, which can confuse developers coming from anonymous-class semantics.
- **Overuse hurts readability** — deeply nested or chained lambdas can become as hard to read as the boilerplate they replace.
- **Type inference limits** — occasionally requires explicit casts or type witnesses when the target functional interface is ambiguous.
- **Checked exceptions are awkward** — functional interfaces generally don't declare checked exceptions, forcing wrapping/rethrowing tricks.
- **Serialization pitfalls** — lambdas are serializable only under specific conditions and can be fragile across JVM versions/refactorings.
- **Performance** — not zero-cost; `invokedynamic`-based instantiation and boxing (for primitive-based functional interfaces without specialized variants) can add overhead in hot paths, though the JIT typically optimizes this well.
...
%------------------------------------------------------------------------------
\section{Inner Classes}\label{sec:InnerClasses}
...
%------------------------------------------------------------------------------
\chapter{Constructors}\label{sec:CodingConstructors}
...
\subsection{Constructor Helpers}\label{sec:ConstructorHelpers}
...
\begin{lstlisting}[numbers=left]
\end{lstlisting}
\lstinline|static final| references to \textit{mutable} objects are no constants (no matter which definition you use), therefore they should never be public – with the final consequence that they are just fields\index{field} – so refer to chapter \tqref{sec:AttributeNames} for their naming.

Some samples for fields\index{field} that are not feasible as constants (because the objects are not constant at all):
\begin{lstlisting}
private static final Map<String,File> m_Files = new TreeMap<String,File>();

// NOT A VALID CONSTANT
private static final Date BEGIN_OF_EPOCHE = new Date( 0 );
...
%------------------------------------------------------------------------------
\hrulefill\newpage 
\section{Resources on the CLASSPATH}\label{sec:ResourcesClasspath}
\autocite{ORACLE_DOC_CLASS:getResourceAsStream}
\autocite{ORACLE_DOC_CLASS:getResource}
\lipsum[1]
...
// AVOID!
d = (a = b + c) + r;        
\end{lstlisting}\lstinline|static final| references to \textit{mutable} objects are no constants (no matter which definition you use), therefore they should never be public – with the final consequence that they are just fields\index{field} – so refer to chapter \tqref{sec:AttributeNames} for their naming.

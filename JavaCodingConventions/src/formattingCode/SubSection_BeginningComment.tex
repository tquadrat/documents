...
Most Java projects does not consist from Java source (\verb#*.java#) files only; aside the already mentioned module definition files (see \tqref{sec:ModuleDefinition}\index{module-info.java}) and the package documentation files\index{package-info.java} (refer to \tqfullvref{sec:PackageDocumentation}), there can be various build files, scripts, resource and property files, and even source files with code from other programming languages.
...
A resource bundle file\index{resource file} \autocite{ORACLE_DOC_RESOURCEBUNDLE_CLASS} can have XML format or it may contain simple key-value pairs. In the latter form, comments are prepended with the hash symbol (``\verb|#|''), and the beginning comment has the following form: 
...
For all versions, \verb#<Year>#, \verb#<Copyright Notice># and \verb#<License Notice># has to be replaced by the appropriate texts. The special character \bdq{\copyright} is used not only because is looks better; it tells you immediately whether your environment is capable to deal with UTF-8 encoded files.

...
\begin{filecontents}{Verbs.tbl}
  ...
  Verb & Contract \\ 
  \hline\endhead
  \makecell{\lstinline|add…( <arg> )| \\ \lstinline|add( <arg> )|} & Add the argument value to an internal list in the object, keeping the argument as it is, so that it could be retrieved again later. Usually, an add-method would not return something. \\ 
  \hline 
  \makecell{\lstinline|append( <arg> )| \\ \lstinline|append…( <arg> )|} & Appends the argument to an extensible data structure, usually by interpreting the argument. Usually, it is not possible to retrieve the argument from the data structure afterwards. This a typical method for a builder, so usually it returns the owning object itself (\lstinline|this|). A sample for this is \lstinline|StringBuffer.append()|. \\ 
  \hline 
 \end{longtable} 
\end{filecontents}
...
call()
This is the main method of a thread that returns a result (an implementation of the Callable interface) and should usually only used in this or a similar context. For details on this, refer to \autocite{CONCURRENT_PACKAGE}.
check()
check( arg )
check...()
Usually this is understood as testing for a given condition, but sometimes it is also used if an object should be marked (as used in “CheckBox”). We do not recommend the latter usage (use mark instead), and because of the chance to misinterpret the verb at all, we do not recommend to use it.
If used, it may not modify the object and it should return a boolean.
clear()
Empties or resets an object.
close()
close...()
Closes the connection to an external resource, like a file or a device.
compose...( arg )
compose...()
Composes a new object or data structure from the (optional) arguments and the internal state of the object, without modifying the object itself. Each call with identical arguments will return the same value.
connect()
connect( arg )
connect...()
connect...( arg )
Creates a connection to an external resource, usually a device or something remote.
create...()
create...( arg )
Create something without modifying the object.
delete()
delete( arg )
Deletes something related to the object, usually external. This may or may not change the state of the object itself. A sample is File.delete().
execute()
execute( arg )
Does something; this is a generic method. Treat the arguments as something like the command line arguments.
exec...()
exec...( arg )
Executes an operation that is named by the additional parts of the name; the arguments are the parameters for this operation.
This is somehow similar to “perform”; nothing is said about the state of the object or the arguments after the method returned.
generate...()
generate...( arg )
generate()
generate( arg )
Generates something based on the objects state, with or without changing it. Each call to generate will have a different result (that distinguishes it from “compose”).
The optional argument may modify the generation process.
get....()
Indicates a getter. It should not take any argument, nor is it changing the state of the object.
The part of the method name after “get” is known as the attribute name; the getter returns the value for that attribute. Refer to “The JavaBeans Technology” [JAVABEANS] for details.
A special variant is the getInstance() method for a Singleton class: here the method is static, and depending on the type of the singleton, it may be responsible for creating the one and only instance of the class.
has...()
This indicates a method that returns a boolean value. If the method takes an argument, this is a key or another kind of reference to an object that can be somehow contained in the object the method is called on, and the method will check if the referenced object exists or not.
Without an argument, the method will just check if a optional reference to a component is null or not.
In both cases, the state of the object is not changed.
initialize()
initialize( arg )
Initialises an object; obviously this will modify the state of the object. Can abbreviated to “init”.
is...()
This is the prefix for a getter for a boolean value. See above for “get”.
load()
load( arg )
...
This chapter lists defined suffices for class names and their function.
Suffix
Description
Adapter
…
Base
A base class of an interface implementation, usually abstract.
ClientAgent
…
Command
An implementation of a command for the Command pattern; use either this or “Operation” as the suffix.
DAO
…
Entity
…
Error
An unchecked exception that is thrown in case of an unrecoverable error. It should not be handled nor handled.
Exception
An exception, checked or unchecked.
Impl
The (default) implementation for an interface.
Listener
An implementation of a listener for the Observer pattern (sometimes also referred to as the “Listener” pattern)
Operation
An implementation of a command for the Command pattern; use either this or “Command” as the suffix.
Service
…
Visitor
An implementation of a visitor for the Visitor pattern
t.b.c.
…

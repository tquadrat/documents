...
The declaration for a record\index{record} \lstinline|Tupel| is shown below:
...
Although not explicitly declared, the record\index{record} class has~…
\begin{itemize}[nosep]
	\item{
\keeplisting{16}
…~a constructor\index{constructor}:\begin{lstlisting}
// Implicitly defined
/**
 *  Creates an instance of a Tupel record class.
 *
 *  @param	key	the value for the 
 *  	{@link #key} 
 *  	record component.
...
The documentation for these elements is generated either based on the arguments for the \bdq{\nameref{sec:TagParam}}\index{param@"@param} tags (see lines~13 and 14 or the \lstinline|Tupel| record source code above) or from standard texts.

The shown use of \bdq{\nameref{sec:TagReturn}}\index{return@"@return} as an inline tag (\bdq{\{\nameref{sec:TagReturn} …\}}) is a shortcut for the documentation of getter methods\index{method!getter method}.

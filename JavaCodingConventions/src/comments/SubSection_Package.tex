...
The documentation documment for a package\index{package}  lives in a dedicated file, the \texttt{package-info.java}\index{package-info.java}, that exists mainly for that comment; refer to \tqfullvref{sec:PackageDocumentation}.
...
The package comment can list the authors of the code, using the \nameref{sec:TagAuthor}\index{author@"@author} tag, the version with the \nameref{sec:TagVersion}\index{version@"@version} tag, when the package\index{package} was created or with which version it was integrated, using the \nameref{sec:TagSince}\index{since@"@since} tag, and other things.
...
The package comment for the main package\index{package!main package} of a project can replace the overview comment (if not the module comment is used for this\footnote{Not all projects will produce modules, so it is possible that your project does not have a module definition file at all.}).

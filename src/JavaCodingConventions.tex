...
When a class, method, constant, field is mentioned the first time in a documentation comment, its documentation should be linked, using the \lstinline|{@link}| or \lstinline|{@linkplain}| tags. This is not necessary if the type is used for a formal parameter or the return value. In these cases, JavaDoc generates these links automatically.

Each \lstinline|{@link}| or \lstinline|{@linkplain}| tag has to placed into a line of its own.
...
The names of classes, methods, constant, fields, parameters etc. as well as \lstinline|null|, \lstinline|true|, and \lstinline|false| have to be written in a monotype font. This can be achieved by encapsulating them in \lstinline|<code>…</code>| HTML tags or placing them inside the JavaDoc \lstinline|{@code}| tag. The monotype font is used automatically for everything inside a \lstinline|{@link}| tag.

Each comment, including each text for a \lstinline|@param|, \lstinline|@return|, and \lstinline|@throws| tag, ends with a full stop.
...
Constants (\lstinline|public static final| fields) that are initialised with a literal have to use the \nameref{sec:TagValue} tag in there description. It should be done also for private or protected static final fields if initialised with a literal.
...
\paragraph{\lstinline|@author|}\label{sec:TagAuthor} Text 

\paragraph{\lstinline|@link|}\label{sec:TagLink} Text 

\paragraph{\lstinline|@param|}\label{sec:TagParam} Text 

\paragraph{\lstinline|@provides|}\label{sec:TagProvides} Text 

\paragraph{\lstinline|@see|}\label{sec:TagSee} Text 

\paragraph{\lstinline|@serial|}\label{sec:TagSerial} Text 

\paragraph{\lstinline|@since|}\label{sec:TagSince} Text 

\paragraph{\lstinline|@uses|}\label{sec:TagUses} Text

\paragraph{\lstinline|@value|}\label{sec:TagValue} Text

\paragraph{\lstinline|@version|}\label{sec:TagVersion} Text

\subsubsection{Custom Tags for JavaDoc}\label{sec:CustomTagsForJavaDoc}
\autocite{TQUADRAT_ORG_FOUNDATION_JAVADOC}

\paragraph{\lstinline|@extauthor|}\label{sec:TagExtAuthor} Text 

\paragraph{\lstinline|@inspired|}\label{sec:TagInspired} Text 

\paragraph{\lstinline|@UMLGraph.link|}\label{sec:TagUMLGraph} Text 

...
\documentclass[11pt,a4paper,titlepage, parskip=half, headsepline, footsepline,cleardoublepage=current]{scrbook}
...
\usepackage[style=numeric,urldate=iso,backend=biber]{biblatex}
\usepackage{nameref}
...
Some of coding guidelines below are different for the type of product or project you are working on. Basically, we can distinguish the following types that will be explained in the following chapters:
\begin{itemize}
\item{Function Libraries}
\item{Feature Libraries}
\item{Tools}
\item{Standalone Applications}
\item{Serverbased Applications}
\end{itemize}

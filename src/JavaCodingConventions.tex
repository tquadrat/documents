...
Examples:\index{java.util.Map}\index{java.util.HashMap}
\begin{lstlisting}[numbers=left]
...
\subsubsection{enum}
\index{emum}An \lstinline|enum| is a special case of a Java class. Usually it only contains the enumeration values, but it can have more components. A skeleton for a \lstinline|enum| class may look like this:
...
//  class MyClass    
\end{lstlisting}
\index{java.time.Instant}\index{java.time.Instant!now()}\index{java.time.Instant!toString()}
...
A custom class that needs to get the data from the net can implement \lstinline|obtainInputData()| as
\begin{lstlisting}
/**
...
public final class MyClass
{
    ...
    public final String process() throws IOException
    {
        ...
        //---* Format the data *-------------------------------------
        final var retValue = formatMethod.apply( resultData, context );
...
But when the called method or constructor stores the provided argument in the instance, this may call for trouble if it keeps just the give reference:
\begin{lstlisting}
public final class MyClass
{
...
You encounter this situation mostly when your method takes a collection type as an argument: should it be \lstinline|Collection|\autocite{ORACLE_DOC_COLLECTION_INTERFACE}\index{java.util.Collection} or \lstinline|ArrayList|\autocite{ORACLE_DOC_ARRAYLIST_CLASS}?

...
//  class MyClass
\end{lstlisting}
The check in line~10 can be omitted if \lstinline|MyClass| is \lstinline|final| or if instances of a derived class can be equal to an instance of the superclass.
...
Samples for this from the Java Runtime Library are
\begin{itemize}
...
This is basically how the IDEs will generate the \lstinline|toString()| method.
...
\autocite{ORACLE_DOC_OBJECT_CLASS:clone}
\subsection{-------------------------------------------- Proceed from here!}
...
How to concatenate two (or more) strings – or other data types to create their representation as a text – is a trade off between readability and performance. This is due to one important characteristic of the class \lstinline|java.lang.String|\autocite{}: it is immutable. This means that
String a = “part1”;
String b = “part2”;
a += b;
will not modify a but return a new String object with the concatenated contents of a and b and assigns a reference to that new object to a.
Older sources described the internal implementation of the + operator for String like this:
String operatorPlus( String a, String b )
{
    StringBuffer buffer = new StringBuffer( a );
    buffer.append( b );
    return buffer.toString();
}
This means that an intermediate object of type StringBuffer has to be created for each concatenation. This gets even worse if you want to append a numerical value to the String, like this:
String a = "part1";
a += 42;
The implementation for this was described as
String operatorPlus( String a, int b )
{
    StringBuffer buffer = new StringBuffer( a );
...
String a = b + c;
and
String a = new StringBuffer( a ).append( b ).toString();
instead of
a += b;
in order to increase performance.
But with each Java version the implementation of + and += for String changed, so that today there is not longer only one recommendation.
5.4.2. Concatenating String Constants
String literals will be always concatenated with the plus operator:
String a = "StringOne" + "StringTwo";
because they are already concatenated during compile time; using StringBuilder1 here would cause negative effects on both performance and readability. This is also true if static final Strings are concatenated with each other or with String literals:
...
The compiler replaces each reference of the static final String by the literal and concatenates them if required.
5.4.3. Concatenating String Variables
...
String a = “part1”;
String b = “part2”;
String s = new StringBuffer( a ).append( b );
...
still being faster than the StringBu*er versions.2
5.4.4. Concatenating Strings in Loops
The picture changes if Strings are extended permanently as done in loops, or in some implementations of the toString() method like below:
// AVOID!!!
...
Here it is the better option to use StringBuilder:
// RECOMMENDED
public String buildSentence( String... words )
{
...
The reason is that even the concatenation with + is faster, it will definitely create more objects that has to be garbage collected later – what will have a negative impact on performance.
5.4.5. Conclusion
The recommendation is to use the + operator for Strings where to combine String constants or String variables in a single statement, but StringBuilder if Strings has to be concatenated in loops or a number of consecutive statements.
When “adding” primitives to a String, these should be translated to Strings first by calling the static toString() method of the appropriate wrapper class first.
This is not necessary if calling the append() methods of StringBuilder as this class has specialised versions of it for each primitive type.
Usually StringBuilder should be preferred over StringBuffer.

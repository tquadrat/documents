...
\keeplisting{33}
It is not recommended to write \texttt{if-else} like this, although it seems to be more conformant to the basic rule:
\begin{lstlisting}
// NOT RECOMMENDED!!
if( <condition1>)
{
...
\item{The opening curly brace for the body statement of the \lstinline|if| and the optional \lstinline|else| clause is on the same indentation level as the keyword itself. In case of the \texttt{if-else} construct, on the level of the \lstinline|else|.}
\item{The statements itself are indented one level relative to the curly braces.}
\end{enumerate}

Please refer also to chapter \tqvref{sec:TheTernaryOperator} that discusses the ternary \texttt{?} operator.

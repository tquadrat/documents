...
See also the chapters \tqfullref{sec:IfStatements}, \tqfullref{sec:SwitchStatement}, and \tqfullref{sec:WhileStatements}.}
...
For the documentation of a package, originally used a file named \verb#package.html#, placed into the package folder. Although this still works, you should use the new \verb#package-info.java# file instead; it looks like this:
...
The \verb#<package description># is a standard JavaDoc comment (refer to the chapter \tqfullvref{sec:DocumentationComments}) and can be as lengthy as necessary.

If the package is annotated\footnote{I recommend to used the \lstinline|@API| annotation even on the package level (refer to chapter \tqfullvref{APIAnnotation} for details).}, the annotations have to be placed directly before the \lstinline|package …| line, and the imports for the annotations follow below.
...
Aside that, no further rules exist. Even more than one character is possible for a name of a type argument: \begin{lstlisting}
...
So Java source code can generally have two kinds of comments:
\begin{itemize}[nosep]
...
Obviously, the advice: “Even if you don't think, a comment might be necessary, add it nevertheless” is the other extrema and equally bad, so writing proper comments remains a complex art, but it follows some rules, and for the rest, we will give some advice.
...
The first line of doc comment (“/**”) for top level classes is not indented; subsequent doc comment lines each have one space of indentation (to vertically align the asterisks). All members, including inner classes, have 4 spaces for the first doc comment line and 5 spaces thereafter (this is congruent for inner classes).
...
Although the current version of Javadoc does not produce correct XHTML (not even correct HTML according to any standard), it is recommended to use correct XHTML syntax inside the comments. This means that tags has to be closed properly and that empty tags like BR or IMG has to be used in there short form. It is also recommended not to use P for line breaks and empty lines in the comment.
The header tags H1 and H2 are used by Javadoc, so for the documentation use only H3 to H6.
Some samples:
// NOT RECOMMENDED
/**
 *  <h1>Master Class
 *  <p>Comment in a new line
 *  …
// OMMIT THE <H1> AND USE <BR/> INSTEAD OF <P>
/**
 *  Master Class.<br/>
 *  <br/>Comment in a new line
 *  …
// NOT RECOMMENDED
 *  …
 *  <ul>
 ...
 *  <img src="doc-files/screenshot.jpg">
 *  …
// CLOSE THE TAGS
 *  …
 *  <ul>
 ...
 *  <img src="doc-files/screenshot.jpg"/>
 *  …
The first sentence of each JavaDoc comment is taken to be placed on an overview, while sentence is defined as everything until the first full stop (“.”) followed by white space, or the first not-inline HTML tag.1 This means that a comment like this
// AVOID!!
/**
 *  <b>Master class.</b>
 *  …
will cause strange output and therefore should be avoided.
It also means that the first sentence should contain a short, summarizing description of the program element that can stand alone for the full description. The documentation for the JDK [JDK_API] provides thousands of examples how this might read.
Some more tips:
    • Each time, a class, method, constant, field is mentioned the first time in a doc comment, its documentation should be linked, using the {@link} or {@linkplain} tags.
    • The names of classes, methods, constant, fields, parameters etc. should be written in a monotype font. This can be achieved by encapsulating them in <code>…</code> HTML tags or placing them into the {@code} tag; it will be done automatically if the {@link} tag is used.
    • Each comment, including each text for a @param, @return, and @throws tag, ends with a full stop.
Some more tips can be found here: “How to Write Doc Comments
for the Javadoc Tool” [JAVADOC_WRITING].
...
These are the easiest comments: they are used to separate the sections or parts of a class (as defined in chapter \tqfullvref{sec:ClassAndInterfaceDeclarations}) from each other. They have the form
\begin{lstlisting}
    /*------------------*\
====** Enum Declaration **===========================================
    \*------------------*/
...
\section{The Annotation @API}\label{APIAnnotation}

...
An \bsq{action} method performs an action, usually specified by the remainder of the name or the arguments for the call.

To some extent, \bsq{action} is synonym to \hyperref[verb:perform]{\bsq{perform}}. Interestingly, the \Java Runtime Library lists a whole series of methods named \lstinline|actionPerformed()|. \\
\hline
...
The method \lstinline|java.lang.StringBuffer::append|\index{java.lang!StringBuffer!append()} \autocite{ORACLE_DOC_STRINGBUFFER_CLASS} is an example for this. \\
\hline
...
An example is \lstinline|Stream.Builder::build|\index{java.util.stream!Stream.Builder!build()} \autocite{ORACLE_DOC_STREAM_BUILDER:build}. \\
\hline
...
The latter interface is important for \texttt{try-with-resources}\index{try-with-resources} and therefore sometimes the method \lstinline|close()| is used to implement some special features; refer to \tqfullvref{sec:TryWithResources}. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|collect()| \\ \lstinline|collect…()|\pslabel{verb:collect}} & 
A \bsq{collect} method collects data and combines it into a single object instance. The source for the data is either determined by the owning object or the arguments; same for the return value that could be determine also by the main part of the name. 

\bsq{collect} is related to \hyperref[verb:gather]{\bsq{gather}}, with the difference that the latter is meant to return a collection of the gathered data. \\
\hline
...
\lstinline|compareTo()|\index{java.lang!Comparable!compareTo()} \autocite{ORACLE_DOC_COMPARABALE:compareTo} is the only method of the interface \lstinline|java.lang.Comparable|\index{java.lang!Comparable} \autocite{ORACLE_DOC_COMPARABALE_INTERFACE}. It compares the owning object with the object provided as the argument and returns the result as an integer, same as a \hyperref[verb:compare]{\bsq{compare}} method. 
...
A \bsq{compile} method translates a \textit{source} (usually a text) into an executable format and returns that. An example for this is \lstinline|Pattern.compile( String )|\index{java.util.regex!Pattern!compile()} \autocite{ORACLE_DOC_PATTERN:compile}. \\
\hline
...
One example is \lstinline|List::copyOf|\index{java.util!List!copyOf()} \autocite{ORACLE_DOC_LIST:copyOf}, another sample is \lstinline|Collections.copy(List,List)|\index{java.util!Collections!copy()} \autocite{ORACLE_DOC_COLLECTIONS:copy}. \\
\hline
...
\lstinline|java.nio.file.Files::delete|\index{java.nio.file!Files!delete()} \autocite{ORACLE_DOC_FILES:delete} is another example; it deletes the file that is specified by the given \lstinline|Path|\index{java.nio.file.Path} \autocite{ORACLE_DOC_PATH_INTERFACE} argument. \\
\hline
...
\makecell{\lstinline|fold( <arg> )| \\ \lstinline|fold…( <arg> )|\pslabel{verb:fold}} & 
The name/prefix \bsq{fold} is mostly synonym to \hyperref[verb:reduce]{\bsq{reduce}}. A method that uses this name or prefix as part of it name should work along the same lines a \hyperref[verb:reduce]{\bsq{reduce}} method. \\
\hline
...
\makecell{\lstinline|gather…()| \\ \lstinline|gather…( <arg> )| \\ \lstinline|gather( <arg> )| \\ \lstinline|gather()|\pslabel{verb:gather}} & 
Like a \hyperref[verb:collect]{\bsq{collect}} method, a \bsq{gather} method collects data, but different from that, it returns a collection with the retrieved data. The source for the data is either determined by the owning object or the arguments; same for the return value that could be determined also by the main part of the name. \\
\hline
...
A method with the name \lstinline|increment()| increases the value of the owning object by 1. If \bsq{increment} is used as prefix to the name, the name indicates the object property whose value is incremented.
...
Depending on the context, a name like \lstinline|loadData()| could be valid, but a some more meaningful name is preferred, like \lstinline|loadClientHistory()| or \lstinline|loadArticleList()|. \\
\hline
...
\begin{tbllisting}[caption=mapFromNull(),xleftmargin=.7cm]
...
An \bsq{obtain} method should never fail (should not throw an exception), but it may return an instance of \lstinline|java.util.Optional|\index{java.util!Optional} \autocite{ORACLE_DOC_OPTIONAL_CLASS}. \\
\hline
...
\makecell{\lstinline|open…( <arg> )| \\ \lstinline|open( <arg> )|\pslabel{verb:open_arg}} & 
...
Methods with this name or with \bsq{pop} as prefix for their name should do something similar. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|print( <arg> )| \\ \lstinline|print()|\pslabel{verb:print}} & 
A \bsq{print} method writes the textual representation of either the argument to the output represented by the owning object, or of the owning object to the output represented by the argument.
...
\makecell{\lstinline|provide…()| \\ \lstinline|provide…( <arg> )|\pslabel{verb:provide}} & 
A \bsq{provide} method returns an attribute of the owning object, similar to a \hyperref[verb:get]{getter method\index{method!getter method}}, but it may generate or modify the returned value before, or the provided reference will allow to modify the attribute. \\
\hline
...
Methods with this name or with \bsq{push} as prefix for their name should do something similar. \\
\hline
...
\makecell{\lstinline|read()| \\ \lstinline|read( <arg> )|\pslabel{verb:read}} & 
A \lstinline|read()| method loads the state of the owning object from an external resource that is usually determined by the state of that object, but it can be also gven as an argument. Usually, the method returns nothing, or it returns that status of the \bsq{read} operation.

But for this purpose, I prefer to use \hyperref[verb:load]{\lstinline|load()|} instead of \lstinline|read()|. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|read…( <arg> )| \\ \lstinline|read( <arg> )| \\ \lstinline|read( <arg> )|\pslabel{verb:read_arg}} & 
A \bsq{read} method reads something from an external source and returns an object constructed from that data. This may or may not change the state of the owning object.

Usually a reference to the external source is provided as a parameter, but it is also possible that this reference is implicit (as for input streams). It is also possible that the destination for the data is given as an argument.

The main part of the name may determine the type of object that is returned. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|reduce( <arg> )| \\ \lstinline|reduce…( <arg> )| \pslabel{verb:reduce}} & 
The method \lstinline|reduce()|\index{java.util.stream!Stream!reduce()} \autocite{ORACLE_DOC_STREAM:reduce} is declared by the interface \lstinline|java.util.stream.Stream|\index{java.util.stream!Stream} \autocite{ORACLE_DOC_STREAM_INTERFACE}. It performs a \textit{reduction} operation \autocite{ORACLE_DOC_STREAM:Reduction} on a \lstinline|Stream|\index{java.util.stream!Stream} – basically it reduces the multiple elements on that stream to a single one.

Your own \bsq{reduce} methods should work along the same lines. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|release…( <arg> )| \\ \lstinline|release( <arg> )|\pslabel{verb:release}} & 
A \bsq{release} method provides the converse operation to an \hyperref[verb:acquire]{\bsq{acquire}} method: it releases the exclusive access to the object that is somehow identified by the given argument. The method returns nothing, or it returns the status of the operation. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|remove…( <arg> )| \\ \lstinline|remove( <arg> )|\pslabel{verb:remove}} & 
A method with the name \lstinline|remove()| or a name that is prefixed with \bsq{remove} will remove something from an internal data structure of the owning object. The given argument is some kind of reference to that thing that should be removed. The method returns nothing, or it returns the removed object or the status of the operation.
    
A \bsq{remove} method is somehow the reverse operation to an \hyperref[verb:add]{\bsq{add}} method. It was already mentioned in chapter \tqfullref{sec:NamesForCollectionMutators}. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|repose( <arg> )|\pslabel{verb:repose}} & 
See \hyperref[verb:sleep]{\bsq{sleep}}. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|reset()| \\ \lstinline|reset( <arg> )| \\ \lstinline|reset…( <arg> )|\pslabel{verb:reset}} & 
A \bsq{reset} method sets the the owning object or an object provided as the argument back to a previously defined state. 

The method \lstinline|java.io.Reader::reset|\index{java.io!Reader!reset()} \autocite{ORACLE_DOC_READER:reset} is an example for this. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|retrieve…()| \\ \lstinline|retrieve…( <arg> )|\pslabel{verb:retrieve}} & 
A \bsq{retrieve} method is similar to a \hyperref[verb:get]{\bsq{get}} method, with the differences that it may change the state of the owning object, that it is not limited in returning a property, and – that makes it also different to an \hyperref[verb:obtain]{\bsq{obtain}} method – that it can fail with an exception. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|run()|\pslabel{verb:run}} & 
\lstinline|run()|\index{java.lang!Runnable!run()} is the name of the main method of all implementations of the interface \lstinline|java.lang.Runnable|\index{java.lang!Runnable} \autocite{ORACLE_DOC_RUNNABLE_INTERFACE}, and as such it is also the main method of all threads except the main thread (there it is \hyperref[verb:main]{\lstinline|main()|}). The name \lstinline|run()| should not be used for methods in a different context. 

Refer also to \autocite{ORACLE_DOC_RUNNABLE:run,ORACLE_DOC_THREAD:run}. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|search( <arg> )| \\ \lstinline|search…( <arg> )|\pslabel{verb:search}} & 
A \bsq{search} method searches something, but it is a valid result that it does not find something; this means, it does not throw an exception if nothing was found. That distinguishes it from a \hyperref[verb:find]{\bsq{find}} method as described above. The method returns an instance of \lstinline|java.util.Optional|\index{java.util!Optional} \autocite{ORACLE_DOC_OPTIONAL_CLASS} with the result.
...
The method \lstinline|save()| is the converse method to \hyperref[verb:load]{\lstinline|load()|}: it persists the current state of the owning object, usually to an external destination. The argument usually determines that destination.

If the method has another return type than \lstinline|void|, the return value is the status of the \bsq{save} operation.
...
A \bsq{save} method provides the converse operation to that of a \hyperref[verb:load_arg]{\bsq{load}} method: it persists the object that is identified by the given argument to an external destination.

If the method has a return type other than \lstinline|void|, this will be the status of the save operation.

The method may or may not change the state of the owning object or the argument. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|select…( <arg> )| \\ \lstinline|select…( <arg> )| \\ \lstinline|select( <arg> )|\pslabel{verb:select}} & 
Usually, a \bsq{select} method is used to chose from a set of options. It will either return a reference to the selected option, or it will change the state of the owning object accordingly. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|shutdown()| \\ \lstinline|shutdown…( <arg> )| \\ \lstinline|shutdown( <arg> )|\pslabel{verb:shutdown}} & 
A \bsq{shutdown} method terminates an asynchronously running operation (usually a thread or a process) gracefully. This operation is either represented by the owning object or by the given arguments. 

Refer also to \hyperref[verb:abort]{\bsq{abort}}, \hyperref[verb:exit]{\bsq{exit}}, \hyperref[verb:cancel]{\bsq{cancel}} and \hyperref[verb:terminate]{\bsq{terminate}}. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|sleep( <arg> )|\pslabel{verb:sleep}} & 
The static method \lstinline|Thread::sleep|\index{java.lang!Thread!sleep()} \autocite{ORACLE_DOC_THREAD:sleep} pauses the \textit{calling} for the period of time specified as the argument. Your implementation of a method with name \bsq{sleep} should do the same.

Unfortunately, \lstinline|Thread::sleep|\index{java.lang!Thread!sleep()} declares to throw \lstinline|InterruptedException|\index{java.lang!InterruptedException} \autocite{ORACLE_DOC_INTERRUPTDEXCEPTION_CLASS}, making the use of that method somehow inconvinient.

The below implementation of a method \lstinline|repose()| makes it easier to pause the current thread:
\index{java.time!Duration}\index{org.tquadrat.foundation.lang!Objects!requireNonNullArgument()}\index{java.time!Instant}\index{java.time!Instant!now()}\index{java.time!Instant!plus()}\index{java.time!Instant!isBefore()}\index{java.lang!Thread}\index{java.lang!Thread!sleep()}\index{java.lang!InterruptedException}\index{java.lang!Thread!currentThread()}\index{java.lang!Thread!isInterrupted()}\index{java.time!Duration!between()}
\begin{tbllisting}[xleftmargin=.7cm,caption=repose()]
public static final boolean repose( final Duration duration )
{
  var retValue = true;
  var remainder = requireNonNullArgument( duration, "duration" );
  var now = Instant.now();
  ...
  SleepLoop: while( retValue && now.isBefore( endTime ) )
  {
    ...
    catch( final InterruptedException _ )
    {
      ...
      now = Instant.now();
      remainder = Duration.between( now, endTime );
    }
  }   //  SleepLoop:
...
A \bsq{split} method breaks up data into pieces, either based on a regular expression, a delimiter or a fixed length. 

The method \lstinline|Pattern::split|\index{java.util.regex!Pattern!split()} \autocite{ORACLE_DOC_PATTERN:split} is an example for an implementation that uses a regular expression. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|supply…()| \\ \lstinline|supply()| \\ \lstinline|supply…( <arg> )| \\ \lstinline|supply( <arg> )|\pslabel{verb:supply}} & 
A supply method returns something, depending on the state of the owning object or always the same. An implementation without parameters can be used as an implementation of the functional interface\index{interface!functional interface} \lstinline|Supplier<T>|\index{java.util.function!Supplier} \autocite{ORACLE_DOC_SUPPLIER_INTERFACE}. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|switch…( <arg> )| \\ \lstinline|switch…()|\pslabel{verb:switch}} & 
A method \lstinline|switch()|, with or without arguments, is invalid because \lstinline|switch| is a keyword of the \Java Programming Language (refer to \autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:Keywords}). Consequently, \bsq{switch} can be used only as the prefix for a method name.
...
\makecell{\lstinline|terminate…()| \\ \lstinline|terminate()| \\ \lstinline|terminate…( <arg> )| \\ \lstinline|terminate( <arg> )|\pslabel{verb:terminate}} & 
Implement a \bsq{terminate} method to finish an operation gracefully. The operation is determined by the owning object or through the given argument.

Refer to \hyperref[verb:abort]{\bsq{abort}}, \hyperref[verb:cancel]{\bsq{cancel}}, \hyperref[verb:exit]{\bsq{exit}} and \hyperref[verb:shutdown]{\bsq{shutdown}} that do similar things. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|toggle…()| \\ \lstinline|toggle()|\pslabel{verb:toggle}} & 
With a method named \bsq{toggle} you will flip the state of an attribute, usually of type \lstinline|boolean|. You can use it also to switch through the values of an \lstinline|enum| attribute.

The method can return the previous value, the current value or just nothing. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|update()| \\ \lstinline|update( <arg> )| \\ \lstinline|update…()| \\ \lstinline|update…( <arg> )|\pslabel{verb:update}} & 
An \bsq{update} method updates either the state of the owning object or an object provided as an argument. In the latter case, it may return the updated object. \\
\hline

%------------------------------------------------------------------------------

\makecell{\lstinline|wait…( <arg> )| \\ \lstinline|wait…()|\pslabel{verb:wait}} & 
The method \lstinline|Object::wait|\index{java.lang!Object!wait()} \autocite{ORACLE_DOC_OBJECT:wait} cannot be overwritten because it is declared final. 

The method \lstinline|wait()| suspends the current thread until that thread is interrupted or notified. Therefore it declares to throw an \lstinline|InterruptedException|\index{java.lang!InterruptedException} \autocite{ORACLE_DOC_INTERRUPTDEXCEPTION_CLASS}. 

Methods that uses the prefix \bsq{wait} in their name should do something similar than \lstinline|Object::wait|\index{java.lang!Object!wait()}. \\
\hline

...
Of course, all those categories are possible triggers for an exception in Java. But this means that we can quite simply generalise the response to an error: \textit{in case of an error condition, throw an exception}!

Consequently, we can also simplify error handling to exception handling. And there are only a few option on how to respond to an exception, although these can combined with each other, to some extent:
\begin{itemize}
\item{Throw an exception and make it an S.E.P.\footnote{S.E.P~=~Someone Else's Problem (refer to \autocite{ADAMS_HITCHHIKERS_GUIDE}).} – particularly useful (no irony!) for libraries.}
...
\item{Abort the current operation – either a thread or the whole program.}
\item{Perform an alternate operation.}
\end{itemize}
...
\item{Whether repeating or retrying a failed operation depends on the kind of operation, and not that much on the type of the product. This also should not be done indefinitely; at some point, it gets useless to repeat or retry a constantly failing operation.}
...
\item{Only the main thread (that one that executes the method \lstinline|main()|) reports to the console when it aborts due to an exception; per default, all other threads will die silently.

Of course this can be changed; chapter \tqvref{sec:Exception Handling in Threads} describes how this is done.}
\end{itemize}

\subsection{Throwing Exceptions}
Your code will emit an exception when it encounters an error condition of some kind. Exceptions are discussed in detail in the Java Language Specification\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:Exceptions}; here I will only repeat the basics.
...
Exception classes that extend \lstinline|java.lang.Exception| are so-called \textit{checked} exceptions; if a method wants to throw one of these, it has to declare that with the \lstinline|throws| clause. \lstinline|java.lang.RuntimeException| is a direct subclass of \lstinline|java.lang.Exception| and the superclass of all \textit{run-time} exceptions – declaring these is not mandatory.
...
When instantiating an exception instance, you should always provide a textual message that describes the error condition and/or provides additional information, like the name of the file that cannot be found, or the SQL statement with the invalid syntax. In this regard, the sample above is bad, because it does not provide such an explanation.

Some exception classes do not have a constructor that allow to provide a cause, or they have constructors that allow only a message or a cause, but not both. A sample for that is \lstinline|java.lang.ExceptionInInitializerError|\autocite{ORACLE_DOC_EXCEPTIONININITIALIZERERROR_CLASS}.
...
\subsection{Exceptions in Threads}\label{sec:Exception Handling in Threads}
\subsection{-------------------------------------------- Proceed from here!}
\begin{lstlisting}
\end{lstlisting}
...
\item{If a constructor calls a method of its own class or a superclass, this method has to be \lstinline|static|, \lstinline|private|, or \lstinline|final|\footnote{The important setting is \lstinline|final|, because \lstinline|private| and \lstinline|static| methods are implicitely \lstinline|final|}.

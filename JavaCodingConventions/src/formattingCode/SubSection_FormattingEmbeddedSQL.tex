...
If your code needs to access a database\index{database} on an RDBMS\index{RDBMS}, it is quite likely that you have to write some SQL\index{SQL} \autocite{ANSI:SQLStandard,W3SCHOOLS:SQL} statements into your code. Even when you are using an ORM\index{ORM} like JPA\index{JPA} \autocite{ECLIPSE_JPA} and Hibernate\index{Hibernate} \autocite{HIBERNATE_ORM}, it might still be necessary to add some SQL\index{SQL} statements or statements in the tool's variant of SQL (HQL\index{HQL} in case of Hibernate\index{Hibernate}).

...
\begin{tabular}{ll}
\rule[-1ex]{0pt}{2.5ex} ‘Old’ \lstinline|switch| & ‘New’ \lstinline|switch| \\ 
\rule[-1ex]{0pt}{2.5ex}
...
As we have two different forms the \lstinline|switch| statement now, it is worth to define some coding guidelines for it.
...
A reason for the JPEs had been the error prone syntax of the original \lstinline|switch| statement; this is fixed by the new form – but only when it is used.}
...
The new form does not allow ‘fall-through’, hence this is only relevant for the old form; ‘fall-through’ is the main reason why the old \lstinline|switch| is considered to be error prone.}
...
\item{Never use the old \lstinline|switch| form for a \lstinline|switch| expression!

Although it works, you should just not do it, because in that case the result will be returned as an ‘argument’ of the \lstinline|break| statement – and can be easily mixed up with a \lstinline|break| label.}
\end{enumerate}

%------------------------------------------------------------------------------

\subsection{Advantages of the new Syntax}
The biggest advantage of the new syntax is that it allows to avoid the problems of the original syntax; previously, you wrote
\begin{lstlisting}[numbers=left]
switch( selector )
{
    case 1: 
...
With the new syntax, you do not need the \lstinline|break| statement anylonger:
\begin{lstlisting}[numbers=left]
switch( selector )
{
    case 1 -> executeOne();
...
The \lstinline|switch| expressions\autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpression, ORACLE_DOC_LANGUAGE_SPECIFICATION:SwitchExpressionBlock, ORACLE_DOC_SWITCHEXPRESSIONS} came with the new syntax, too, but it is not dependend on it; you can write as well
\begin{lstlisting}
// AVOID!!
final var result = switch( selector )
{
    case 1: break retrieveOne();
    
    case 2: break retrieveTwo();
    
    default: break retrieveDefault();
}
\end{lstlisting}
instead of
\begin{lstlisting}
...
\end{lstlisting}
I recommend to use always the new syntax, at least for \lstinline|switch| expressions.

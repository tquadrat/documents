...
A common requirement is that code must be self-explaining. In chapter \tqfullref{sec:Goals}, I included on page \pageref{par:ReadableCode} a few quotes that I would like to repeat here:
...
So we know now that human readers of a program are more important than the computer that will execute the code.
...
Comments will also provide information that the code contains only implicitely, or that it does not contain at all. These could be dependencies to external configurations or information about type and allowed values for parameters. This is generally grouped under the term \bdq{program documentation} – we will come back to this later.
...
\begin{itemize}
	\item{Comments should not be enclosed in large boxes drawn with asterisks or other characters, with the exception of structuring comments as described in \tqfullref{sec:FileStructure} and \tqfullref{sec:StructuringComments}.}

	\item{Comments should never include control characters such as tabulator, form-feed and backspace or alike. In Javadoc\index{javadoc} comments only (see \hyperref[sec:DocumentationComments]{below}), most non-ASCII\index{ASCII} characters should be escaped with their HTML\index{HTML} equivalent.}

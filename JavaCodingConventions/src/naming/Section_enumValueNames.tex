\section{Names for \bsq{enum} Values}\label{sec:EnumValues}
...
\end{lstlisting}
where \lstinline|DIR_| and \lstinline|ALIGN_| as the respective prefixes, differentiating the \verb#LEFT# for the direction from that from the alignment.
...
\begin{enumerate}[label=P\arabic*.]
	\item{The names of \lstinline|enum| values\index{enum} are all uppercase, with underscores (\verb#_#) as separators between words.}

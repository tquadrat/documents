...
Some feature libraries can even be started standalone, in a server mode. Samples for this are the H2 Database Engine\autocite{H2_DATABASE}\index{H2}, ActiveMQ\autocite{APACHE_ACTIVEMQ}\index{Apache ActiveMQ}, or Jetty\autocite{ECLIPSE_JETTY}\index{Eclipse Jetty}.
...
In the gray area here I would place JUnit\autocite{JUNIT5}\index{JUnit} and Log4j\autocite{APACHE_LOG4J}\index{Log4j}.
...
\subsection{Mutable Return Types}\label{MutableReturnTypes}
...
But keep in mind that changes to the state of the object may be reflected also to the return value if your methods returns a reference to a mutable attribute:
...
\paragraph{Line~11} creates a instance of \lstinline|java.util.ArrayList|\autocite{ORACLE_DOC_ARRAYLIST_CLASS}\index{java.util!ArrayList} for the return value and initialises that with \lstinline|m_Attribute|. The returned value will be mutable, but changes will not affect the attribute and therefore will not affect the state of the object instance, too. And \lstinline|null| values are no problem, other than for the solution with \lstinline|copyOf()|.

\paragraph{Line~12} basically combines lines~10 and 11, eliminating the shortcoming of the solutions from lines~9 and 10.

\paragraph{Line~13} ‘clones’\index{java.lang!Object!clone()} \lstinline|m_Attribute|; obviously this works only for instances that implement \lstinline|java.lang.Cloneable|\autocite{ORACLE_DOC_CLONEABLE_INTERFACE}\index{java.lang!Cloneable}, but although collections (and arrays) are cloneable themselves, they may contain elements that are not cloneable, causing a \lstinline|CloneNotSupportedException|\autocite{ORACLE_DOC_CLONENOTSUPPORTEDEXCEPTION_CLASS}\index{java.lang!CloneNotSupportException}.

In the examples, the collection's element type is immutable\footnote{\lstinline|java.lang.String|\index{java.lang!String} is immutable.}; if the elements would be mutable, you need to take the corresponding precautions for them, too. Basically that would mean a deep copy.

In general you have to take care that you will not return a reference to an attribute with a mutable type that would allow to modify the internal state of the owning object. This clearly does not affect just collections – although this is the most common case – but for example also instances of \lstinline|java.util.Date|\autocite{ORACLE_DOC_DATE_CLASS}\index{java.util!Date}\footnote{Another good reason to abandon \lstinline|java.util.Date| and to use the classes from the \lstinline|java.time|\index{java.time} package instead.}. Obviously, for other types the details on how to create a copy or to make them immutable will differ.
...
\subsection{Special Return Values}
%------------------------------------------------------------------------------

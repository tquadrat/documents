...
Except InvalidArgumentException and exceptions derived from that (“validation exceptions”), a domain or component may never emit an exception that is not defined by itself (except NPE, but this may not occur anyway). All other exceptions (exceptions, not errors!) must be caught and handled or wrapped. InvalidArgumentExceptions are thrown only by the API methods of the component as response of an invalid argument to that method. IAE thrown by other component used by this component must be wrapped and delegated or handled internally.
 
To illustrate that, we assume that we defined the component “Printing” that is responsible for the generation of documents for paper-like output and their distribution. It defines the exception class “PrintingException” (or “PrintingRuntimeException”, if you like that better), derived from java.lang.RuntimeException. If will directly extend RuntimeException, or  if Printing has a parent component, it will extend that component’s domain specific exception. Perhaps you have defined an application exception (like our PRRuntimeException), so the domain specific exceptions will extend that.
 
This “domain root exception” allows all consumers of the respective component to catch all specific exceptions from that component with a single catch:
...
In this sample, „recipientID“ and „dataSource“ will enhance the information that is already provided with the PrintingException object; probably the Printing component was called with the data itself (not with the dataSource), and the recipient is not part of the generated document.
...
There are libraries that use “Error” (domain specific implementations of java.lang.Error) to signal the abort condition, while “Exception” means: “you can try again with different (valid) input data”. I don’t like this approach as it is a widely accepted best practice not to catch Error.

...
The break in the default case is redundant, but it prevents a fall-through error if later another case is added – although it should be assured that the default case is always the last case in the switch statement.
...
switch( <condition> )
{
    ...
    case GHI: <singleStatement>; break;
...
Every time a case falls through (does not include a break statement), add a comment where the break statement would normally be. This is shown in the preceding code example with the “// Falls through!” comment. Another comment assigned to the switch statement then should describe the expected sequence of cases.
Every switch statement should include a default case, even when using an enum type as selector and handling all known values in the cases; in such a case, the default can throw an exception about an unknown value. The break in the default case is redundant, but it prevents a fall-through error if later another case is added – although it should be assured that the default case is always the last case in the switch statement.
The use of braces in the cases is optional, but sometimes useful as it allows to declare additional local variables for a given branch.

\subsubsection{General Rules for switch}

2.5.7. for Statements
...
This means that braces has to be used in any case when more than one statement has to be executed in the loop. In addition, this single statement has to be written completely into the same line as the for. Examples:
for( int i = 0; i < max; ++i ) sum += i; // OK
for( String s : texts ) System.out.println( s ); // OK
// DISCOURAGED
for( int i = 0; i < max; ++i ) if( v [i] > 0 ) sum += v [i]; // ??
// DISCOURAGED
for( String s : texts ) if( !s.empty() ) out.println( s ); // ??
for( int i = 0; i < max; ++i ) if( v [i] > 0 ) // AVOID!!!
{
    sum += v [i];
}
else
...
for( String s : texts ) if( !s.empty() ) // AVOID!!!
{
    System.out.print( “Value: “ );
    System.out.println( s );
}
The versions above marked with the double question marks are correct according to this rule, but as said in comment above them, their use is discouraged and therefore should be avoided, too.
An empty for statement (one in which all the work is done in the initialization, condition, and update clauses) should have the following form:1
for( <initialization>; <condition>; <update> );
When using the comma operator in the initialization or update clause of a for statement, avoid the complexity of using more than three variables. If needed, use separate statements before the for loop (for the initialization clause) or at the end of the loop (for the update clause). 

\subsection{while Statements}\label{sec:WhileStatements}
A while statement should have one of the following form:
while( <condition> )
{
    <statements>;
}
while( <condition> ) <singleStatement>
Again, braces can only be omitted in case there is only a single statement, written on the same line than the while statement that has to be executed in the loop.
An empty while statement should have the following form: 
while( <condition> );
2.5.9. do-while Statements
...
In case a finally block is present, the catch block is optional, meaning that no exception is caught:
try
{
    <statements>;
}
...
Note: An empty catch block is unacceptable under all circumstances! Refer to chapter “5.6.2 General Exception Handling” on page 76 how to handle exceptions. See also chapter “4.2.3 Single-Line Comments” on page 61 about empty blocks and comments.
An empty finally block is completely obsolete and should be removed.

...
\begin{itemize}
\item{\texttt{naming/DomainGlossary.tex}}
\item{\texttt{naming/DomainGlossary.tbl.tex}}
\end{itemize} 
...
\subparagraph{Bounded Context:} [\textit{Which context this definition applies to}]

\subparagraph{Definition:} [\textit{Clear, precise definition as agreed upon by domain experts and dev team}]

\subparagraph{Synonyms/Aliases:} [\textit{Other names used for this concept, if any – flag if inconsistent}]

\subparagraph{Type:} [\bdq{Entity} | \bdq{Value Object} | \bdq{Aggregate} | \bdq{Aggregate Root} | \bdq{Domain Event} | \bdq{Domain Service} | \bdq{Policy} | \bdq{Specification} | \bdq{Enum/Category}]

\subparagraph{Attributes/Properties:} [\textit{Key attributes, if it's a model element}]

\subparagraph{Related Terms:} [\textit{Links to other glossary entries this term relates to or depends on}]

\subparagraph{Example:} [\textit{Concrete example in plain business language}]

\subparagraph{Code Reference:} [\textit{Class/interface name in the codebase – keeps code and language in sync}]

\subparagraph{Notes/Constraints:} [\textit{Business rules, invariants, edge cases}]

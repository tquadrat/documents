...
Eclipse users can use the function \texttt{Source|Organize Imports} from the menu, or the short-key \texttt{Shift+Ctrl+O}. This will reorder the import statements, explode the wildcards and remove unused imports; it will even add currently missing imports – given that this is properly configured in the \texttt{preferences} at \texttt{Java|Code Style|Organize Imports}.

At IntelliJ~IDEA, the function \texttt{Code|Optimize Imports} from the menu does something similar; the short key would be \texttt{Ctrl+Alt+O}.

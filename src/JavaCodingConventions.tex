...
\item{For WebServices: JAX\_WS\autocite{IBM_JAX_WS, ECLIPSE_XML_SERVICES}\index{JAX_WS}}
...
This is faster than the first version, although the concatenation with \verb#+# is usually faster than using \lstinline|StringBuilder|\index{java.lang!StringBuilder}, because the shown version will definitely create much less objects that have to be garbage collected later – what will have a negative impact on performance.
...
\subsection{Conclusion}
The recommendation is to use the \verb#+# operator for strings where to combine literals, \lstinline|String| constants and/or String variables in a single, standalone statement, but to consider \lstinline|StringBuilder|\index{java.lang!StringBuilder} or even \lstinline|StringJoiner|\index{java.lang!StringBuilder} if strings have to be concatenated in loops or (large) a number of consecutive statements.\footnote{But keep \autocite{Knuth:PrematureOptimization} in mind, where Donald E. Knuth said something about “premature optimisation”.}

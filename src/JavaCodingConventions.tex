...
Looking a the code to write, all three works basically the same:
\begin{enumerate}[nosep]
\item{During the initialisation of your class, obtain an instance of a logger class; the instances are named, usually by the name of the the class that obtains it.}
...
The predefined log levels differ:
\begin{filecontents}{LogLevels.tbl}
  ...
  \hline 
  WARNING & WARN & WARN & An event that might possible lead to an error. \\ 
  \hline 
...
With a log message much more information can be provided to the user than through printing to the console or to a message window. But logging subsystems like Log4J\autocite{APACHE_LOG4J} are usually configured only for feature libraries and standalone applications. Application servers provides a logging facility, too.

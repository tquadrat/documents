...
\end{lstlisting}
In order to understand this it is necessary to remember that \lstinline|name| \textit{is technically not a variable of type 'String'}, but a variable of type 'Reference to String'\footnote{Although the general usage of course corresponds to the first variant.}.
So \lstinline|final| only forbids you from changing the reference a variable holds, it doesn't protect you from changing the internal state of the object it refers to by using its public API:
\begin{lstlisting}
...
\end{lstlisting}
would mean that you cannot call any methods on \lstinline|object| that would change the internal state of the instance. ‘Safe’ methods are marked as \lstinline|const|:
\begin{lstlisting}[language=C++]
class MyClass
{
...
\end{lstlisting}
Therefore the call to \lstinline|unsafeMethod()|
\begin{lstlisting}[numbers=left,language=C++]
const MyClass object;
object.unsafeMethod(); // does not compile
\end{lstlisting}
in line~2 does not work, but this variant will do:
\begin{lstlisting}[numbers=left,language=C++]
MyClass object;
object.unsafeMethod();
\end{lstlisting}
Unfortunately, it is not that simple in Java.

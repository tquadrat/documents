...
A method, in particular a non-\lstinline|private| method, should return the most general interface whenever possible. This means \lstinline|List| and not \lstinline|ArrayList|, \lstinline|Map| and not \lstinline|HashMap|, and \lstinline|InputStream| and not \lstinline|FileInputStream|.
...
Basically it is not bad if a method returns a mutable type, but it can get nasty.
...
\end{lstlisting}
5.9. 
The formal parameter of a method should always be defined with the most general type that still matches its function. This depends on how the argument is processed by the method. So it is possible to use CharSequence instead of String, List or even Collection instead of ArrayList or Vector, and Map instead of HashMap or Hashtable.1
It is preferred that a method returns an interface instead of the implementing class. So it should return List instead of LinkedList (or even better Collection), Map instead of Hashtable, and so on.
In case a collection is returned, it should be considered to return an array instead, especially if the returned set is not meant to be modified. This increases the type safety again.
Next the returned values should be either immutable or copies of the internal objects if it is not explicitly intended to return a reference to the internal object. A comment about the nature of the return value is always useful.
As stated above, a collection can be returned as an array. If it is to large for that, or for a map, the collection can be made immutable by calling one of
    • java.util.Collections.immutableList()
    • java.util.Collections.immutableSet() or
    • java.util.Collections.immutableMap()
with it.
If the objects inside the collection are mutable themselves, it should be considered to clone them. The same yields for single mutable objects.
Methods should neither define returns values from the type Object, nor should they define arguments of that type. Instead it should be considered to define an interface that can be used instead. We are aware of the fact that this may not be possible always.

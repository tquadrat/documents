...
\index{java.io!BufferedReader}\index{java.nio.files!Files!newBufferedReader()}\index{java.util!List}\index{java.util!List!of()}
\begin{lstlisting}
// ACCEPTABLE
...
And if you want to avoid the typing, use a \lstinline|record|\autocite{ORACLE_DOC_RECORD,ORACLE_DOC_LANGUAGE_SPECIFICATION:RecordClasses} instead. Sometimes also an instance of \lstinline|java.util.Map|\index{java.util!Map} or another collection implementation could be an alternative to a specialised class.
...
When you call that constructor, there is a lot of strings on the arguments list, and when you miss a name, suddenly the new person got the person id of a contact as its first name. This problem was already discussed in the chapters \tqfullvref{sec:SwapToCompilerErrors}, \tqfullref{sec:MultipleSameTypeArgs} and \tqfullvref{sec:GenericParameterTypes}, and using value types were suggested as a solution.

Another issue is that you have to validate the provided person ids before storing them. Most probably this happened already elsewhere, perhaps already several times, but the constructor is public and you have to do it again.

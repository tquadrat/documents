...
Exception classes that extend \lstinline|java.lang.Exception|\index{java.lang!Exception} are so-called \textit{checked} exceptions\index{exception!checked}\index{checked exception|see{exception!checked}}; if a method wants to throw one of these, it has to declare that with the \lstinline|throws| clause. The class \lstinline|java.lang.RuntimeException|\index{java.lang!RuntimeException} is a direct subclass of \lstinline|java.lang.Exception|\index{java.lang!Exception} and the superclass of all \textit{runtime}\index{runtime exception|see{exception!unchecked}} or \textit{unchecked}\index{exception!unchecked}\index{unchecked exception|see{exception!unchecked}} exceptions – declaring these is not mandatory.
...
\subsubsection{Generics}\label{sec:GenericTypes}

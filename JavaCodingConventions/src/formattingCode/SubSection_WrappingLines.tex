...
\keeplisting{11}
When another method call is used as an argument\footnote{Usually, you should avoid to call another method in an arguments list, but sometimes this cannot be avoided (refer to \tqfullvref{sec:CodingConstructors} and there to section \tqfullref{sec:ConstructorHelpers}). In most cases, introducing a local variable is the better option.}:
\keeplisting{9}
\begin{lstlisting}

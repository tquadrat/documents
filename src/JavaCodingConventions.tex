...
If an exception was not caught in your code, it ”bubbles up” until it reaches the main method for the current thread (that is \lstinline|main()| for the main or program thread, and \lstinline|run()| for any other thread). If this method will not handle the exception, the thread will be aborted. If this thread is the last non-deamon thread, the JVM will be terminated, too.
...
The output looks like this\footnote{When using an older version of Java, no output from the thread wold be shown.}
\begin{verbatim}
...
This is acceptable here because the program ends after the closing curly brace of the \lstinline|catch|-block!
\subsection{-------------------------------------------- Proceed from here!}

When a Java We are used to the behaviour that the JVM displays an exception on System.err that is not handled somewhere, and then to terminate the program.
Unfortunately, this is only the case for the main thread (that one that calls the main() method of a program). The default behaviour for any other thread is to die silently.
Although we do not want to encounter any problems in a thread, we want to know when any issue arises. Java provides several solutions to this.
The first one is to catch all exceptions in the run() method of the thread:
\begin{lstlisting}
public void run()
{
    try
...
\end{lstlisting}
This seems to be in contradiction to the statements in chapter 5.6.2 above, but here it is allowed as it is the last chance to do something with this exception that will be otherwise vanish in limbo.
Nevertheless, we do not recommend the approach above, only for small, tool-like programs. Instead it should be considered to provide an implementation of Thread.UncaughtExceptionHandler.
This could be done in three ways:
    1. Each single thread can get its own uncaught exception handler by calling setUncaughtExceptionHandler() on it, with an instance of an implementation of Thread.UncaughtExceptionHandler as the argument.
    2. On creation of a thread, a ThreadGroup object will be assigned to it (if none is explicitly set, it will be the group of the current thread); its uncaughtException() method is called in the case the thread itself does not have an uncaugth exception handler assigned to it. To change the behaviour of that standard handler it is necessary to derive a new class from ThreadGroup and to overwrite the uncaughtException() there.
    3. By calling the Thread.setDefaultUncaughtExceptionHandler(), an instance of an implementation of Thread.UncaughtExceptionHandler can be installed for all threads. It will be called as a last resort.
A sample implementation for an uncaught exception handler is provided in the chapter 6.7 on page 150. For details refer to the documentation in [UNCAUGHT\_EXCEPTION\_HANDLER].

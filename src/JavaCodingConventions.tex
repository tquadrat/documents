...
\textbf{Note:} For Log4j, the integer value for the highest log level is closer to \lstinline|0| (zero), while for JDK~Logging, it is closer to \lstinline|Integer.MAX_VALUE|.

With Log4j, you can achieve a similar result by using \lstinline|Logger::always|\autocite{APACHE_LOG4J:Logger.always} instead of creating a new log level:
\begin{lstlisting}
…
m_Logger.always()
    .withThrowable( e )
...
As said, the methods above are implemented in the class \lstinline|org.tquadrat.foundation.lang.Objects| that also provides reimplementations of the methods from \lstinline|java.util.Objects| that now throw \lstinline|NullArgumentException| instead of \lstinline|NullPointerException|.
...
\begin{itemize}
\item{\lstinline|checkFromIndexSize()| Checks if a \lstinline|fromIndex| value fits into a given range for a given size of a sub-range.
...
The method \lstinline|Arrays::copyOfRange| will of course check the arguments by itself, but the main goal of the sample code above is to illustrate how to use the methods.}

\item{\lstinline|checkFromToIndex()| Checks if a \lstinline|fromIndex| and a \lstinline|toIndex| value fits into a given range.
...
\item{\lstinline|checkIndex()| Checks if the given value is within the bounds of the range from 0 (inclusive) to the given length (exclusive).
...
\item{\lstinline|requireValidArgument()| Applies a given validation on the given value, and if that validation fails, a \lstinline|ValidationException| with a specified message is thrown.
...
//  classMyClass
\end{lstlisting}
If the code of the class will not modify the attribute, you can of course use \lstinline|List::copyOf|\index{java.util.List!copyOf()}, too.
...
%------------------------------------------------------------------------------
\subsection{General vs. specific Argument Types}
You should choose that type for a formal parameter of a method or a constructor that provides just the functionality that is needed.
...
\lstinline|java.lang.Iterable|\autocite{ORACLE_DOC_ITERABLE_INTERFACE}\index{java.lang.Iterable}

\lstinline|java.util.Collection|\autocite{ORACLE_DOC_COLLECTION_INTERFACE}\index{java.util.Collection}

\lstinline|java.util.List|\autocite{ORACLE_DOC_LIST_INTERFACE}\index{java.util.List}

\lstinline|java.util.Set|\autocite{ORACLE_DOC_SET_INTERFACE}\index{java.util.Set}

\lstinline|java.util.ArrayList|\autocite{ORACLE_DOC_ARRAYLIST_CLASS}\index{java.util.ArrayList}

\lstinline|java.util.LinkedList|\autocite{ORACLE_DOC_LINKEDLIST_CLASS}\index{java.util.LinkedList}

\lstinline|java.util.Vector|\autocite{ORACLE_DOC_VECTOR_CLASS}\index{java.util.Vector}

\lstinline|java.util.HashSet|\autocite{ORACLE_DOC_HASHSET_CLASS}\index{java.util.HashSet}

\lstinline|java.util.LinkedHashSet|\autocite{ORACLE_DOC_LINKEDHASHSET_CLASS}\index{java.util.LinkedHashSet}

\lstinline|java.util.SortedSet|\autocite{ORACLE_DOC_SORTEDSET_INTERFACE}\index{java.util.SortedSet}

\lstinline|java.util.EnumSet|\autocite{ORACLE_DOC_ENUMSET_CLASS}\index{java.util.EnumSet}

\lstinline|java.util.TreeSet|\autocite{ORACLE_DOC_TREESET_CLASS}\index{java.util.TreeSet}
\newline\includegraphics{collection}\newline

\lstinline|java.util.Map|\autocite{ORACLE_DOC_MAP_INTERFACE}\index{java.util.Map}

\lstinline|java.util.HashMap|\autocite{ORACLE_DOC_HASHMAP_CLASS}\index{java.util.HashMap}

\lstinline|java.util.SortedMap|\autocite{ORACLE_DOC_SORTEDMAP_INTERFACE}\index{java.util.SortedMap}

\lstinline|java.util.LinkedHashMap|\autocite{ORACLE_DOC_LINKEDHASHMAP_CLASS}\index{java.util.LinkedHashMap}

\lstinline|java.util.Hashtable|\autocite{ORACLE_DOC_HASHTABLE_CLASS}\index{java.util.Hashtable}

\lstinline|java.util.EnumMap|\autocite{ORACLE_DOC_ENUMMAP_CLASS}\index{java.util.EnumMap}

\lstinline|java.util.TreeMap|\autocite{ORACLE_DOC_TREEMAP_CLASS}\index{java.util.TreeMap}
\newline\includegraphics{map}\newline
...
You should prefer \lstinline|java.lang.CharSequence|\autocite{ORACLE_DOC_CHARSEQUENCE_INTERFACE} over \lstinline|java.lang.String|\autocite{ORACLE_DOC_STRING_CLASS} for the type of method and constructor arguments, even if you in fact need a \lstinline|String|: calling \lstinline|toString()| on an instance of \lstinline|String| is a \verb#NOOP# that will be optimised away by the \verb#javac# compiler or by the JIT, but line~3 from the sample below is easier to read than line~5, and the latter does not provide any additional information to the reader.
\begin{lstlisting}[numbers=left]
...
Of course, there will be always methods that will be called never with an instance of \lstinline|CharBuffer|\autocite{ORACLE_DOC_CHARBUFFER_CLASS} or \lstinline|StringBuilder|\autocite{ORACLE_DOC_STRINGBUILDER_CLASS} as an argument, but only with \lstinline|String| instances, just because of the context the are defined in. Then it does not make any sense at all to define the formal parameter as \lstinline|CharSequence|.
...
There are exceptions to this rule, in particular for internal code, but in general it must be taken care that a modification to a returned value does not have an unwanted impact to the status of the object that owned the accessor method.}\label{princ:TypesForArgumentAndReturnValues:P1}

\item{Chose the return type based on what the caller may want to do with the return value.

Together with \ref{princ:TypesForArgumentAndReturnValues:P1} this can result in return types for accessor methods that are completely unrelated to the types of the associated attributes.}
...
It is important to know, that in case a \lstinline|Set| is a \lstinline|SortedSet|\autocite{ORACLE_DOC_SORTEDSET_INTERFACE} or a \lstinline|Map| is a \lstinline|TreeMap|\autocite{ORACLE_DOC_TREEMAP_CLASS}, the sort order gets lost for the copy.

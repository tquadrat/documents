...
This document was compiled using \TeX/\LaTeX\index{TeX}\index{LaTeX}\footnote{In case it is relevant for you, I used \bsq{TexMaker} as the editor for the \LaTeX~sources. I run it on Mac and Linux, but the software is also available for Windows. TexMaker can be downloaded from this location: \url{https://www.xm1math.net/texmaker/download.html}; on most Linux distributions it can installed directly from the repositories.}, and its source can be found on GitHub\index{GitHub} \autocite{TQUADRAT_ORG_DOCUMENT_REPOSITORY}. The diagrams were generated with Graphviz\index{graphviz} \autocite{Graphviz} and GreenLightning's ‘Nassi-Shneiderman Diagram Generator’\index{Nassi-Shneiderman Diagram Generator}\index{NSDG|see{Nassi-Shneiderman Diagram Generator}} \autocite{WIKIPEDIA:NSD, NSDG, NSDG:Download}.

\subsection{The Document Structure}
This document itself consists of four major parts, aside of this \bdq{\nameref{sec:Introduction}} and the \bdq{\nameref{sec:Appendices}} (containing additional information, references and examples).

\paragraph{\bdq{\nameref{sec:FormattingTheSourceCode}}} deals with proper formatting of the Java source files and additional files that are part of your project.

\paragraph{\bdq{\nameref{sec:NamingConventions}}} covers the naming of the program elements (modules, packages, classes, methods, fields, constants, local variables, formal parameters).

\paragraph{\bdq{\nameref{sec:WritingProperComments}}} discusses some guidelines for writing proper comments, also for the generation of the documentation of the code.
...
At the opposite, the source code examples in the chapter \tqfullvref{sec:Examples} obey all rules, follow all guidelines and implement all recommendations.

...
All the rules, guidelines and recommendations in this document assume that you use at least Java~17 – see the language specification in \autocite{ORACLE_DOC_LANGUAGE_SPECIFICATION} – to write and to compile your code.  Of course this version was also used for the examples.
...
As we have an unlimited line length, it will not happen that an expression does not fit into a single line. But you might consider to break long lines to improve the readability. In this case, you should follow these general principles:
\begin{itemize}
...
\end{enumerate}
And in general, this sequence is ordered by the syntax for Java source files, although the comments are not mandatory, and the package defintion and the imports are optional~…
...
Only one of these classes or interfaces can be \lstinline|public|, the others have to be package-local (meaning the modifiers \lstinline|private| and \lstinline|protected| are not allowed). If you really have more than one class in a single source file, the \lstinline|public| one should be the first entry, all others will follow, sorted alphabetically by their names.
...
Technically, records and enums are special types of classes, while an annotation is somehow a special interface.
...
The \lstinline|return| statement has to be the last statement of a method, while the \lstinline|yield|\index{yield} statement has to be the last statement of a \lstinline|case| block; refer to the chapter \tqfullvref{sec:ReturningValues} for more details on this.

When \lstinline|return| is always the last statement of a method, it means that the simple form
\begin{lstlisting}
return;
\end{lstlisting}
is obsolete\footnote{\lstinline|yield|\index{yield} alone (without a return value) in a \lstinline|case| block will always cause a compiler error.}.

The use of parentheses with the \lstinline|return| or \lstinline|yield|\index{yield} statement is not required unless they make the return value more obvious in some way.
...
\subsubsection{The new Form of ‘switch’}\label{sec:NewSwitch}
...
\begin{tabular}{ll}
\rule[-1ex]{0pt}{2.5ex} “\verb#case L:#” or ‘old’ \lstinline|switch| & “\verb#case L ->#” or ‘new’ \lstinline|switch| \\ 
\rule[-1ex]{0pt}{2.5ex}

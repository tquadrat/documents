...
The name of a method needs to describe what that method does; that is that contract between developer and maintainer mentioned in \ref{p:NamesAsConctract}
...
This is of course only one possible approach, and is does not work in all cases. So it does not allow to add additional attributes, on the other side it can be used to introduce some nasty side effects that are not immediately visible from the code of the class.
...
\item{An argument should be as general as possible.

You should choose that type from the class hierarchy as the argument type that provides all the functionality you really need.}

\item{Do not cast the provided argument values.

If you need to cast the given argument value, you have most probably chosen the wrong argument type. But again, there are exceptions to the rule – especially if you have to implement an interface or to override a method. \lstinline|Object::equals| is a good sample for such an exception from the rule.}

\item{Chose the return type based on what the caller may want to do with the return value.

Together with \ref{princ:TypesForArgumentAndReturnValues:P1} this can result in return types for accessor methods that are completely unrelated to the associated attributes.}

...
As Java does not allow the use of \lstinline|goto|\index{goto}\footnote{… despite it is a reserved keyword.}, labels\index{label} are mainly used with the \lstinline|break|\index{break} and \lstinline|continue|\index{continue} statements. Consequently, labels will be assigned mostly to \lstinline|switch|\index{switch} (the ``\verb#case L:#'' variant only), \lstinline|for|\index{for}, \lstinline|while|\index{while} and \lstinline|do-while|\index{while!do} statements.

...
\subsubsection{Property Types, Constructor Arguments and Return Values for Accessors}
...
Unfortunately, the class \lstinline|java.util.Date|\autocite{ORACLE_DOC_DATE_CLASS}\index{java.util!Date} is \textit{not} immutable. And the although we copied the array, it can still be modified through reflection.
...
//  class MyClass
\end{lstlisting}
The implementation of \lstinline|java.util.List|\autocite{ORACLE_DOC_LIST_INTERFACE}\index{java.util!List} that is returned by \lstinline|List::of|\autocite{ORACLE_DOC_LIST:of}\index{java.util!List!of()} does not provide any API that would allow to modify it. Nevertheless, it is not safe from being modified through reflection, if someone would spent enough time and effort to research how to do it. This means that it is difficult to get a collection really secured against this kind of abuse, but on the other side, you should not make it \textit{too} easy!

%------------------------------------------------------------------------------

\subsubsection{Mutators for Immutable Types}
...
\subsubsection{\textit{Apparently} immutable Types}
...
\index{java.lang!Override}\index{java.lang!Object!hashCode()}\index{org.tquadrat.foundation.lang!Objects!isNull()}\index{java.lang!Integer!valueOf()}\index{org.tquadrat.foundation.lang!Objects!hash()}\index{java.lang!Objects!toString()}
\begin{lstlisting}[numbers=left]
public final MyClass
{
...
\subsubsection{Changes through Reflection}
...
But you do not need to think about malicious code introduced by hackers; too often your own colleagues can be worse!
...
That the objects of the ‘immutable’ class \lstinline|MyClass| could be manipulated through reflection was not the only fault in this scenario, but that one that made it possible.
